...
**2026-09-07 execution override:** the current Codex task owns the whole completion mission under the Producer's new instruction. The current finite sequence and evidence are at the top of `docs/PROJECT_STATE.md` and Issue #172. Historical role handoffs, narrow path locks and routine technical approval waits below do not override that instruction. Locked product decisions and legacy-data preservation remain; only the final Producer approval unlocks integration/merge/tag/Release/official Pages, followed by public verification. The persistent goal cannot be completed at a test, documentation or release-candidate checkpoint.
...
Current execution: Section138.36 `QA_HARNESS_STAGE25_CONTACT_TARGET_CARD_STARVATION / SOL_REMEDIATION`. PR171 HEADab1a41c automatic run33917360399 passes PR Verify, enemy6/6, Hosted WebKit11/11, Stage 6 Phase G and Stage 24 Phase G. Required Phase G then stops at Stage 25's unchanged45-second engagement setup boundary because its normal opening/sustain paths deploy guardian once and repeatedly select the first recovered DOM card, medic. The retained production transaction records guardian id2 completing a real impact on shield id5, but later support-object shields are in a different production lane with attackSequence0 and no human target. Required `zombie:red-panther-shield` receipt remains missing; product/AI/balance/WebKit/receipt code are unchanged owners0.

The first source-correct local invocation is setup-invalid before browser launch because its inherited browser path does not exist. After binding the existing workspace WebKit, the first actual process stops at2.150s on the correctly selected Stage25 map: the enabled visible CTA was clicked, but the native-save-backed map-to-formation transition had no target/event action in the runner's immediate first check. The runner incorrectly failed before using its existing240-turn bound. Transaction, PNG and diagnostics0 are retained. Correct only this async settlement ownership inside the same matrix/test paths: a no-action turn waits the existing18ms and rechecks, while turn240 remains terminal with last state. Functional delayed/terminal tests are required; no product/storage, click count or combat budget changes.

The next material Stage25 process reaches the full exact formation but the `スキップ` source locator becomes unstable/detaches during that target transition; Playwright rejects after its unchanged30-second action wait while failure state and PNG already show formation, diagnostics0. Correct only target handoff ownership: one normal source click remains; its rejection is accepted only if the exact requested stop selector is visible, otherwise the original error is rethrown. Apply to skip/advance, with functional positive/negative tests. No retry, force click, timeout or phase/body substitution.

The exact five-path correction now passes focused source/Design `57/57`. One fresh standalone Stage25 WebKit process passes `1/1`, binding shield source id4/sequence1 to human guardian id2/direct impact0/hit and the exact attack cue/audio receipt; diagnostics0 and both cleanup owners succeed. The unchanged ordered Stage6 ->24 ->25 route subsequently passes three fresh processes, `9/9` total, with every transaction success/`COMPLETE`/diagnostics0/cleanup success. Report hashes and all nine transaction receipts are fixed in Design138.36. A PowerShell outer-wrapper redirection diagnostic occurred only after the three npm processes had completed and written those reports; direct artifact readback is authoritative and no browser rerun was performed.

Final local source gates pass focused57/57, production build/full1424/1424, lint0 errors/12 existing warnings, content validation, static Pages generation and diff check. Local static metadata is Version1.0.0 with the precommit parent SHA only; it is not release evidence. Exact hashes remain in Design138.36.

ROLE_LOCK SOL_REMEDIATION. Old-HEAD run33917360399 terminated naturally on PR171 HEADab1a41c with all66 jobs terminal: success65, failure1, cancelled0 and skipped0. Its sole failure is the already-classified old-head `V1 Phase G Production Matrix`; every other job, including the complete canonical WebKit viewport matrix, succeeded. No cancel, retry or promotion occurred. Status `LOCAL_SECTION13836_COMPLETE / OLD_HEAD_RUN_TERMINATED / READY_FOR_LIVE_PREFLIGHT`. Live GitHub preflight now gates one exact-five-path normal commit and fast-forward push, followed by a distinct all-required automatic CI. Then resume full30-Stage progression, integrated local54/unfiltered remote54, macOS PWA, physical Safari, native audio/mobile/human gates and fixed-HEAD SOL_FINAL_REVIEW. Public profile ID60032464-d706-4708-a7c8-055ec03c694d stays unopened. `PRODUCT_DESIGN_CHANGE: 0`; M3NO. The sole FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT and explicit-approval-only release tail remain unchanged.
...
CURRENT INDEPENDENT WORKTREE: codex/v100-save-boundary-6faa at lifecycle base6faabdf1ee7d9b8a513f451b7f9a9dca2e38a97b. Section138.18 / [Issue172#5538846335](https://github.com/SUSANO-OOO/Zombieee/issues/172#issuecomment-5538846335) owns the exact six-path validation of legacy shape/integrity/history and the eight safe settings. ROLE/NEXT_OWNER SOL_REMEDIATION, NO ACTIVE LUNA HANDOFF. Section138.17 support changes and audio failure remain preserved in their separate worktree; they are not accepted or overwritten. CI953 remains active on88bea5f; no push/cancellation. The following lifecycle paragraph records its completed base, not an instruction to reimplement it.
...
Canonical Design Lock remains V100-SOL-DL-001 r114, current source packet Section138.22. Earlier packet cursors below are completed history, superseded by the current paragraph above. NO ACTIVE LUNA HANDOFF. ROLE/NEXT_OWNER SOL_REMEDIATION.
...
RESUME_FROM: Section138.18 existing strict inspector/import parser + actual-play evidence + field validation -> executable positive/negative tests and unchanged existing V1 regression/design gates -> six-path diff audit -> preserve the isolated normal source commit. The first15/17 fixture gate and exact schema/namespace correction remain recorded in Issue172#5538917655. Reconcile this source packet with the support candidate only after its separate failure is resolved; then run integrated full build/runtime gates. Do not claim IndexedDB entitlement, popup ownership, old-key isolation or actual settings UI from these pure tests.
