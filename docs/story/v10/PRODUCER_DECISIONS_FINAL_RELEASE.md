...
## 2026-09-26 スマートフォン実プレイ後の改訂

ProducerのStage 5付近までの実機試遊を受け、従前の戦闘波数・ボス出現時刻・CAPS数値・育成表示・爆薬ドラムの仕様は製品判断として更新する。全イベント脚本は進行と人物identityを維持して編集し、戦闘は敵の密度とボス前の緊張を上げる。育成ではHP、防御、近接／射撃ダメージ、移動速度、攻撃間隔と強化差を表示する。新しい隊員の配備登録は解禁時にポップアップで案内する。新規セーブの画質初期値は最高にする。中央の支援は普通のドラム缶へ戻し、専用画像と爆発しない遮蔽物として実装する。タイトルに権利・クレジット入口を置く。スマートフォンの視認性を保つ範囲でanimation/VFXも改善する。

これらは下記の旧数値・旧「爆薬ドラム缶」記述に優先する。stable IDと既存セーブの購入・進行・設定は保持する。全30ステージの確認は開発者モードで進行と内容を通し、通常操作は代表場面で検証する。正式公開、Ready、最終merge、tag、Release、Issue closeはProducer最終承認まで行わない。

表示台詞の現行正本は`docs/story/v10/STORY_SCRIPT_V100_PRODUCER_REWRITE.md`とその生成物`app/v100StoryEvents.js`。従前のv10全文SHAは経緯とビジュアル制作基準として保持し、旧台詞を現在の表示へ戻さない。脚本の全イベントはStage順、人物identity、ENDING、台詞なし11枚のエンドロール、EPILOGUEを維持する。
...
### 2026-09-09 ローカル画像処理と実装分担の追加確定

Producerは、クマパーソンの防御絵や社長等の透過不良を、無料のローカル画像処理で修正することを承認。元画像を保管し、顔・髪・服装・武器等のidentityを変えず、背景の残りと身体の誤透過を修復する。過去checkpointの「方法確認は未回答」は当時の記録であり、この回答で方法確認は解消した。画像の最終採否と正式公開の承認は別に維持する。

使用量と効率を常に考慮し、親担当は設計・確認・レビュー、明確になった実装はLunaが担当する。必要な範囲だけを引き継ぎ、同じファイルの無調整な並行編集、完了した調査やテストの無目的な繰り返しを避ける。

### 2026-09-09 音楽方向の追加確定

スマートフォン再制作へのProducer回答として、試聴ページの通常戦闘「Simulacra」と、くまや・落ち着いた日常会話「Amberlight」の方向性を承認。既存boss BGMを維持する。他場面をこの2曲で埋めず、作戦準備、危機、不穏、救出後の安堵、喪失、終幕の用途を分け、再利用可能な場面群として制作・配置する。曲は台詞送りごとに再始動せず、場面の転換で切替える。

無料かつ配布許諾の確認できる音源をゲーム用に加工して使用し、作者・曲名・ライセンス・加工内容をクレジットへ表示する。この承認は2案の音楽方向のみ。その他の曲・実機ミックス・全体品質・画像処理・正式公開の最終承認を兼ねない。
