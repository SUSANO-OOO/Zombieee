# Solへ送る一括指示 — Version 1.0.0正式リリース設計 v2
...
- event主要人物は腰上portrait、battle participantはbattle atlas必須です。
- 既存assetを先に監査し、`NEW_REQUIRED`だけを有限個生成します。
- 新規identity masterは1画像1character、全身完全表示、武器・装備・手足を切らない、透明背景、文字・logo・枠・背景なし。candidateをcharacter sheetへまとめません。
- RED PANTHERはsurvival knife近接兵、盾兵、SMG兵、指揮官兵の四兵種。共通装備を持たせつつ、体格、armor、武器、silhouetteで判別可能にします。
...
単なる巨大化、色違い、装甲追加、genericなAI怪物、無関係な別monster、綺麗すぎる近未来robotは不合格です。「約2倍」を厳密倍率にせず、既存TAKUYAとの連続性、final boss圧力、mobile readability、telegraph、hitbox、animation、performanceを満たすexact scaleをSolが決定します。
...
- legacy記念CAPS等をreload、recovery、manual import、multiple tabsで二重付与しません。
...
- boss exact設計とcross-mode mapping
- Segawa、TAKUYA-Ω、RED PANTHER、変異ムガリアン社長を含むfinite asset inventory、identity lock、selected master、provenance
...
このmissionではproduction code、runtime asset統合、save変更、main直接push、merge、tag、Release、Pages公開、Issue closeを行いません。
