...
候補コードには、場所を識別できる3ステージのキャンペーン、導入から結末までの物語・通信イベント、専用タイトル／作戦室／3戦場／案内役のproduction visual、9曲のBGMと100種のSE、初期4人・進行解放2人、星・報酬・解放の一度限り適用、開始・続行・確認付き初期化、schema version 2のローカルセーブ、約25%の戦場拡張、標的制約、敵死亡・灰化、非数値表現の味方感染・汎用ゾンビ化・火炎焼却、日本語主体の主要表示、iPhone Safariのvisual viewport・safe area対応が含まれる。音源は109論理assetすべてに旧iPhone Safari向けMP3とOGGの2sourceを持つ。数値、候補方式、暫定表現、実ブラウザの検証基準は[PRODUCT_SPEC_0.6.0.md](PRODUCT_SPEC_0.6.0.md)、素材の出典・権利・加工・最終ハッシュは[THIRD_PARTY_ASSETS.md](THIRD_PARTY_ASSETS.md)を参照する。
...
- [Issue #24](https://github.com/SUSANO-OOO/Zombieee/issues/24) Audio：後続追跡Issue。0.6.0向けproduction BGM・SE・音声ミキサー候補はIssue #29 / Draft PR #30へ統合済みだが、物理端末の聴感・スピーカー確認は未完了
...
- 物理iPhone Safariでのスピーカー聴感、長時間性能、発熱：未確認。ローカル実ブラウザと自動検証の結果は[QA_0.6.0.md](QA_0.6.0.md)を参照する
