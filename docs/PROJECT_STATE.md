...
- release requestは`version`と`issue_number`を記録できるが、現行workflow／public QAがrelease単位の契約として動的利用していない
- 固定release SHAへ暗黙依存する箇所がある

PR #47では、文書mergeによって公開版がdocs SHAへ切り替わらないよう、`.github/pages-release-request.json`をimmutable Version 0.7.0へ更新した。正式URL上の最終metadata確認はR0の入口で再実施する。
