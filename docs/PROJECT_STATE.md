...
GitHub `main`の現在のHEADはGitHubまたはGitで都度確認する。この文書には「最新main SHA」を固定値として記録しない。
...
正式な開発コードは、プロデューサーが承認したGitHub `main`上のコミットである。作業開始時は、GitHubまたは次のGit情報を使って現在のrefを確認する。

- ローカルの現在ブランチとHEAD
- `origin/main`の追跡状態
- 作業ツリーの未コミット・未追跡変更
- 対応Issueが指定する基準コミットまたはref
...
3. GitHub `main`の現在のHEADとローカルの追跡状態を確認する。
