...
- PR #27のopen／Draft／merged状態、baseとhead、現在head SHA、CI runとcheck結果などの変動値は、GitHub APIまたはGitHub画面から都度取得し、この文書へ固定複製しない。
- PR #27の変更候補には、[AGENTS.md](../AGENTS.md)の二段階承認方式、GitHub Actions CI、[RELEASE_BACKUP_RECOVERY.md](RELEASE_BACKUP_RECOVERY.md)のリリース・バックアップ・復元手順が含まれる。
...
- ゲーム名変更、Combat Polish、Audioは、それぞれ[Issue #25](https://github.com/SUSANO-OOO/Zombieee/issues/25)、[Issue #23](https://github.com/SUSANO-OOO/Zombieee/issues/23)、[Issue #24](https://github.com/SUSANO-OOO/Zombieee/issues/24)の未実装事項であり、PR #27の変更候補へ含めない。
- PR #27の採否、マージ、`main`反映などの結果は、GitHub上で事実が成立した後に正式所有元へ記録し、成立前に完了済みとして先回り記載しない。
