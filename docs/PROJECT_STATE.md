...
- 基準：0.5.0固定復元基準SHA
- Draft PR：[PR #27](https://github.com/SUSANO-OOO/Zombieee/pull/27)
- Draft PR base：`main` / `466cec62230e774ee4f25d31988906400412e228`
- Draft PR head branch：`ops/codex-autonomous-workflow`
- Draft PR初回head SHA：`f3903aee93d75547c31a36ea90155b6c44a2d7d9`。独立レビュー修正でheadは更新されるため、現在値はPRとIssueから都度取得する
- 現在工程：運用基盤のDraft PR #27（CI・独立レビュー修正反復を含む）
- 停止地点：Draft PR未マージ
- GitHub `main`：変更しない
...
Draft PR #27では、workflow名`CI`、job ID`verify`、job表示名`Verify`で、push runとpull request runがともに成功した。Checks APIで実測した`CheckRun.name`は`Verify`、appはGitHub Actionsであり、required status check候補は`Verify`である。workflow名をcontextのprefixとして加えない。
