...
特定の実行能力や外部機能が利用不能でも、全体を停止しない。依存しないフェーズを継続し、該当レーンだけを一件のcapability blockerとして、欠けている能力、試した安全な代替、影響する成果物、再開条件とともに報告する。仮画像、単純ビープ、未検証placeholderで完了を偽装しない。

禁止：force push、共有履歴のrebase・amend、`main`直接push、identity lockまたは実画面品質ゲート未通過画像の正式採用、repository visibility変更、secrets・課金・外部契約の無断変更。
...
- 画像は選定済みidentity baseから必要なportraitとbattle atlasだけを連続生成し、不要な差分・立ち絵を先行生成しない
...
- identity lockされた人物・敵・背景の特徴が派生画像と実ゲームで維持される
...
主観的な見た目は自動テストだけで保証しない。2026-07-19のプロデューサー決定に基づく自動採否、identity lock、派生比較、実ゲーム統合証拠、段階レビューで担保する。

## 6. 実行フェーズ

各フェーズは出口証拠が揃うまで完了扱いにしない。

### P0：同期・baseline・能力・公開可否

作業：

1. `origin/main`とfeature branchをfetch
2. PR #38のstate、draft、base、head、mergeability、CIを確認
3. 最新`origin/main`をfeature branchへ通常のmerge commitで統合。rebase・squash禁止
...
9. GitHubのbranch push、PR更新、Ready化、merge、tag、Release、Pages操作の権限・手段を確認
10. Chromium、WebKit、画像生成・編集、音響制作・ライセンス確認、artifact保存の利用可能性を確認
11. 利用不能な能力がある場合、依存レーン、代替手段、継続可能なレーン、再開条件を記録

出口証拠：merge commit、baseline結果、対象モジュール、既知不具合、PR #39取込み、匿名公開見込み、capability matrix、blockerと並行計画。

P0の調査・計画を完了せず本実装へ入らない。能力不足が判明しても、調査結果と並行計画が確定すれば、影響しないP1〜P5を進めてよい。
...
P1〜P5と並行可能。個別承認待ちでは停止せず、identity lockと内部品質ゲートを通過した画像を順次統合する。

画像キュー：初期所有人物、Stage 1〜2解放人物、Stage 4〜6人物、いくらちゃん、新敵、Stage 4〜6背景・オブジェクト、重要イベントカット、必要な保全4名派生。

最初の画像制作前に`docs/ASSET_APPROVALS_0.7.0.json`を作成する。過去の個別承認履歴と、承認制撤回後のproducer-delegated自動採否を同じmanifestへ記録し、管理文書を乱立させない。

各画像はAsset ID、種別、対象、用途、予定パス、revision、元identity Asset ID、短い意図、スマホ識別点、内部採否、品質証拠を記載する。

選定済み基準アートの顔、年齢感、髪、体格、服、配色、武器、装備、姿勢、シルエットをidentity lockとする。派生画像は元画像との比較証拠を添える。

内部採用だけで統合完了としない。844×390と844×340の実ゲームで、同一人物、識別性、視線、武器構造、手の把持、切れ、接地、UI干渉、背景との調和を確認する。

出口証拠：manifest、review mode、revision、commit、最終パス、SHA、派生比較、実ゲーム統合画像。過去の個別承認分だけ承認URLを保持する。

### R3：視覚統合レビュー

採用アセットのidentity lock、UI、背景、接地、スプライト、スマホ視認性、Stage差別化を独立reviewし、High／Medium未解消0とする。
...
- 全新規画像がproducer-delegated内部品質ゲート通過
...
2. PRをReady化
3. expected head SHAを固定し、merge method `merge`でPRを通常merge。squash・rebase禁止
...
- capability matrixと未使用・未確認能力
- 画像採否一覧、review mode、identity比較
