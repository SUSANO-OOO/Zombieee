...
test("player-facing surfaces share one immutable Version 0.9.7 identity", async () => {
  assert.deepEqual(RELEASE_IDENTITY, {
    version: "0.9.7",
    tag: "v0.9.7",
    label: "Version 0.9.7",
    title: "西新世紀末物語｜アーリーアクセス版 0.9.7",
  });
  assert.equal(RELEASE_VERSION, "0.9.7");
  assert.equal(RELEASE_TAG, "v0.9.7");
  assert.equal(RELEASE_LABEL, "Version 0.9.7");
  assert.equal(RELEASE_TITLE, "西新世紀末物語｜アーリーアクセス版 0.9.7");
