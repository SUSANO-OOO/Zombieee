...
test("player-facing surfaces share one immutable Version 0.9.9.5 identity", async () => {
  assert.deepEqual(RELEASE_IDENTITY, {
    version: "0.9.9.5",
    tag: "v0.9.9.5",
    label: "Version 0.9.9.5",
    title: "西新世紀末物語｜アーリーアクセス版 0.9.9.5",
  });
  assert.equal(RELEASE_VERSION, "0.9.9.5");
  assert.equal(RELEASE_TAG, "v0.9.9.5");
  assert.equal(RELEASE_LABEL, "Version 0.9.9.5");
  assert.equal(RELEASE_TITLE, "西新世紀末物語｜アーリーアクセス版 0.9.9.5");
  ...
  assert.doesNotMatch(game, /externalSession \? "Version 1\.0\.0" : RELEASE_LABEL/u);
});
