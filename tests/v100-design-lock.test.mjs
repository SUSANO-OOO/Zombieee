...
test("v1.0.0 design documents bind one immutable Design ID and baseline", async () => {
  ...
  assert.match(design, /Revision: `r105`/u);
  assert.match(design, /Status: `DESIGN_LOCKED`/u);
  assert.match(handoff, /Canonical Design Lock: `V100-SOL-DL-001 r105`/u);
...
test("r11 preserves r8-r10 ownership and closes monotonic Stage 24 causal history", async () => {
  ...
  assert.match(projectState, /current Design Lock：`V100-SOL-DL-001 r105`/u);
...
test("r12-r97 lock SOL actionability, WebKit lifecycle, asset ownership, causal continuity, and one final Producer checkpoint", async () => {
  ...
  const r63 = design.match(/## 79\. Revision r63([\s\S]+?)(?=## 80\. Revision r64)/u)?.[1] ?? "";
  const r64 = design.match(/## 80\. Revision r64([\s\S]+?)(?=## 81\. Revision r65)/u)?.[1] ?? "";
  const r65 = design.match(/## 81\. Revision r65([\s\S]+?)(?=## 82\. Revision r66)/u)?.[1] ?? "";
  const r66 = design.match(/## 82\. Revision r66([\s\S]+?)(?=## 83\. Revision r67)/u)?.[1] ?? "";
  const r67 = design.match(/## 83\. Revision r67([\s\S]+?)(?=## 84\. Revision r68)/u)?.[1] ?? "";
  const r68 = design.match(/## 84\. Revision r68([\s\S]+?)(?=## 85\. Revision r69)/u)?.[1] ?? "";
  const r69 = design.match(/## 85\. Revision r69([\s\S]+?)(?=## 86\. Revision r70)/u)?.[1] ?? "";
  const r70 = design.match(/## 86\. Revision r70([\s\S]+?)(?=## 87\. Revision r71)/u)?.[1] ?? "";
  const r71 = design.match(/## 87\. Revision r71([\s\S]+?)(?=## 88\. Revision r72)/u)?.[1] ?? "";
  const r72 = design.match(/## 88\. Revision r72([\s\S]+?)(?=## 89\. Revision r73)/u)?.[1] ?? "";
  const r73 = design.match(/## 89\. Revision r73([\s\S]+?)(?=## 90\. Revision r74)/u)?.[1] ?? "";
  const r74 = design.match(/## 90\. Revision r74([\s\S]+?)(?=## 91\. Revision r75)/u)?.[1] ?? "";
  const r75 = design.match(/## 91\. Revision r75([\s\S]+?)(?=## 92\. Revision r76)/u)?.[1] ?? "";
  const r76 = design.match(/## 92\. Revision r76([\s\S]+?)(?=## 93\. Revision r77)/u)?.[1] ?? "";
  const r77 = design.match(/## 93\. Revision r77([\s\S]+?)(?=## 94\. Revision r78)/u)?.[1] ?? "";
  const r78 = design.match(/## 94\. Revision r78([\s\S]+?)(?=## 95\. Revision r79)/u)?.[1] ?? "";
  ...
  const r85 = design.match(/## 101\. Revision r85([\s\S]+?)(?=## 102\. Revision r86)/u)?.[1] ?? "";
  const r86 = design.match(/## 102\. Revision r86([\s\S]+?)(?=## 103\. Revision r87)/u)?.[1] ?? "";
  const r87 = design.match(/## 103\. Revision r87([\s\S]+?)(?=## 104\. Revision r88)/u)?.[1] ?? "";
  const r88 = design.match(/## 104\. Revision r88([\s\S]+?)(?=## 105\. Revision r89)/u)?.[1] ?? "";
  const r89 = design.match(/## 105\. Revision r89([\s\S]+?)(?=## 106\. Revision r90)/u)?.[1] ?? "";
  const r90 = design.match(/## 106\. Revision r90([\s\S]+?)(?=## 107\. Revision r91)/u)?.[1] ?? "";
  const r91 = design.match(/## 107\. Revision r91([\s\S]+?)(?=## 108\. Revision r92)/u)?.[1] ?? "";
  const r92 = design.match(/## 108\. Revision r92([\s\S]+?)(?=## 109\. Revision r93)/u)?.[1] ?? "";
  const r93 = design.match(/## 109\. Revision r93([\s\S]+?)(?=## 110\. Revision r94)/u)?.[1] ?? "";
  const r94 = design.match(/## 110\. Revision r94([\s\S]+?)(?=## 111\. Revision r95)/u)?.[1] ?? "";
  const r95 = design.match(/## 111\. Revision r95([\s\S]+?)(?=## 112\. Revision r96)/u)?.[1] ?? "";
  const r96 = design.match(/## 112\. Revision r96([\s\S]+?)(?=## 113\. Revision r97)/u)?.[1] ?? "";
  const r97 = design.match(/## 113\. Revision r97([\s\S]+?)(?=## 114\. Revision r98)/u)?.[1] ?? "";
  const r98 = design.match(/## 114\. Revision r98([\s\S]+?)(?=## 115\. Revision r99)/u)?.[1] ?? "";
  const r99 = design.match(/## 115\. Revision r99([\s\S]+?)(?=## 116\. Revision r100)/u)?.[1] ?? "";
  const r100 = design.match(/## 116\. Revision r100([\s\S]+?)(?=## 117\. Revision r101)/u)?.[1] ?? "";
  const r101 = design.match(/## 117\. Revision r101([\s\S]+?)(?=## 118\. Revision r102)/u)?.[1] ?? "";
  const r102 = design.match(/## 118\. Revision r102([\s\S]+?)(?=## 119\. Revision r103)/u)?.[1] ?? "";
  const r103 = design.match(/## 119\. Revision r103([\s\S]+?)(?=## 120\. Revision r104)/u)?.[1] ?? "";
  const r104 = design.match(/## 120\. Revision r104([\s\S]+?)(?=## 121\. Revision r105)/u)?.[1] ?? "";
  const r105 = design.match(/## 121\. Revision r105([\s\S]*)$/u)?.[1] ?? "";
  ...
  const r63Handoff = handoff.match(/## 72\. Revision r63([\s\S]+?)(?=## 73\. Revision r64)/u)?.[1] ?? "";
  const r64Handoff = handoff.match(/## 73\. Revision r64([\s\S]+?)(?=## 74\. Revision r65)/u)?.[1] ?? "";
  const r65Handoff = handoff.match(/## 74\. Revision r65([\s\S]+?)(?=## 75\. Revision r66)/u)?.[1] ?? "";
  const r66Handoff = handoff.match(/## 75\. Revision r66([\s\S]+?)(?=## 76\. Revision r67)/u)?.[1] ?? "";
  const r67Handoff = handoff.match(/## 76\. Revision r67([\s\S]+?)(?=## 77\. Revision r68)/u)?.[1] ?? "";
  const r68Handoff = handoff.match(/## 77\. Revision r68([\s\S]+?)(?=## 78\. Revision r69)/u)?.[1] ?? "";
  const r69Handoff = handoff.match(/## 78\. Revision r69([\s\S]+?)(?=## 79\. Revision r70)/u)?.[1] ?? "";
  const r70Handoff = handoff.match(/## 79\. Revision r70([\s\S]+?)(?=## 80\. Revision r71)/u)?.[1] ?? "";
  const r71Handoff = handoff.match(/## 80\. Revision r71([\s\S]+?)(?=## 81\. Revision r72)/u)?.[1] ?? "";
  const r72Handoff = handoff.match(/## 81\. Revision r72([\s\S]+?)(?=## 82\. Revision r73)/u)?.[1] ?? "";
  const r73Handoff = handoff.match(/## 82\. Revision r73([\s\S]+?)(?=## 83\. Revision r74)/u)?.[1] ?? "";
  const r74Handoff = handoff.match(/## 83\. Revision r74([\s\S]+?)(?=## 84\. Revision r75)/u)?.[1] ?? "";
  const r75Handoff = handoff.match(/## 84\. Revision r75([\s\S]+?)(?=## 85\. Revision r76)/u)?.[1] ?? "";
  const r76Handoff = handoff.match(/## 85\. Revision r76([\s\S]+?)(?=## 86\. Revision r77)/u)?.[1] ?? "";
  const r77Handoff = handoff.match(/## 86\. Revision r77([\s\S]+?)(?=## 87\. Revision r78)/u)?.[1] ?? "";
  const r78Handoff = handoff.match(/## 87\. Revision r78([\s\S]+?)(?=## 88\. Revision r79)/u)?.[1] ?? "";
  ...
  const r85Handoff = handoff.match(/## 94\. Revision r85([\s\S]+?)(?=## 95\. Revision r86)/u)?.[1] ?? "";
  const r86Handoff = handoff.match(/## 95\. Revision r86([\s\S]+?)(?=## 96\. Revision r87)/u)?.[1] ?? "";
  const r87Handoff = handoff.match(/## 96\. Revision r87([\s\S]+?)(?=## 97\. Revision r88)/u)?.[1] ?? "";
  const r88Handoff = handoff.match(/## 97\. Revision r88([\s\S]+?)(?=## 98\. Revision r89)/u)?.[1] ?? "";
  const r89Handoff = handoff.match(/## 98\. Revision r89([\s\S]+?)(?=## 99\. Revision r90)/u)?.[1] ?? "";
  const r90Handoff = handoff.match(/## 99\. Revision r90([\s\S]+?)(?=## 100\. Revision r91)/u)?.[1] ?? "";
  const r91Handoff = handoff.match(/## 100\. Revision r91([\s\S]+?)(?=## 101\. Revision r92)/u)?.[1] ?? "";
  const r92Handoff = handoff.match(/## 101\. Revision r92([\s\S]+?)(?=## 102\. Revision r93)/u)?.[1] ?? "";
  const r93Handoff = handoff.match(/## 102\. Revision r93([\s\S]+?)(?=## 103\. Revision r94)/u)?.[1] ?? "";
  const r94Handoff = handoff.match(/## 103\. Revision r94([\s\S]+?)(?=## 104\. Revision r95)/u)?.[1] ?? "";
  const r95Handoff = handoff.match(/## 104\. Revision r95([\s\S]+?)(?=## 105\. Revision r96)/u)?.[1] ?? "";
  const r96Handoff = handoff.match(/## 105\. Revision r96([\s\S]+?)(?=## 106\. Revision r97)/u)?.[1] ?? "";
  const r97Handoff = handoff.match(/## 106\. Revision r97([\s\S]+?)(?=## 107\. Revision r98)/u)?.[1] ?? "";
  const r98Handoff = handoff.match(/## 107\. Revision r98([\s\S]+?)(?=## 108\. Revision r99)/u)?.[1] ?? "";
  const r99Handoff = handoff.match(/## 108\. Revision r99([\s\S]+?)(?=## 109\. Revision r100)/u)?.[1] ?? "";
  const r100Handoff = handoff.match(/## 109\. Revision r100([\s\S]+?)(?=## 110\. Revision r101)/u)?.[1] ?? "";
  const r101Handoff = handoff.match(/## 110\. Revision r101([\s\S]+?)(?=## 111\. Revision r102)/u)?.[1] ?? "";
  const r102Handoff = handoff.match(/## 111\. Revision r102([\s\S]+?)(?=## 112\. Revision r103)/u)?.[1] ?? "";
  const r103Handoff = handoff.match(/## 112\. Revision r103([\s\S]+?)(?=## 113\. Revision r104)/u)?.[1] ?? "";
  const r104Handoff = handoff.match(/## 113\. Revision r104([\s\S]+?)(?=## 114\. Revision r105)/u)?.[1] ?? "";
  const activeHandoff = handoff.match(/## 114\. Revision r105([\s\S]*)$/u)?.[1] ?? "";
  const currentProcess = projectState.match(/## 4\. 実行体制 — V1 SOL single-owner override([\s\S]+?)## 5\./u)?.[1] ?? "";
  const currentCursor = projectState.match(/## 6\. Version 1\.0\.0 execution cursor — r105 Section 121([\s\S]+?)### Post-V1/u)?.[1] ?? "";
  ...
  assert.match(activeHandoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(currentCursor, /SAME_GATE_REPEAT_COUNT`: `21`/u);
  ...
  for (const source of [r63, r63Handoff, projectState]) {
  ...
  for (const source of [r64, r64Handoff, projectState]) {
    assert.match(source, /QA_HARNESS_SCOPE_INCOMPLETE \/ STAGE6_ONLY_PRESENTATION_QUIESCENCE_LEFT_STAGE24_25_PRE_PROOF_CANVAS_STREAM_UNBOUNDED \+ WEBKIT_ANON_PIPE_BACKPRESSURE \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /EARLY_COMMON_WEBKIT_PRE_PROOF_PRESENTATION_QUIESCENCE \/ ARM_AFTER_OBSERVER_BEFORE_SETUP \+ RELEASE_AFTER_REAL_SETUP \+ FRESH_POST_RELEASE_PROOF_EPOCH \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /e2f8056ff9ac8e454606a3efafcf42d886957a93/u);
    assert.match(source, /37775f92df421f4eb2e2ab7138ed7a6dbede657e/u);
    assert.match(source, /iteration 24/u);
    assert.match(source, /iteration 25/u);
  }
  for (const source of [r65, r65Handoff, projectState]) {
    assert.match(source, /QA_HARNESS_RING_CURSOR_ALIAS \/ POST_QUIESCENCE_AUDIO_LENGTH_BASELINE_SATURATED_AT_128_AND_LOST_NEW_REQUESTS_AFTER_ROTATION \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /MONOTONIC_AUDIO_EPOCH_CUTOFF \/ PAGE_CLOCK_REQUEST_AT_FILTER \+ SATURATED_RING_REGRESSION \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /e2f8056ff9ac8e454606a3efafcf42d886957a93/u);
    assert.match(source, /37775f92df421f4eb2e2ab7138ed7a6dbede657e/u);
    assert.match(source, /iteration 24/u);
    assert.match(source, /iteration 25/u);
  }
  for (const source of [r66, r66Handoff, projectState]) {
    assert.match(source, /COMMON_WEBKIT_PRESENTATION_LIFETIME_CLOSURE \/ PHASE_G_SINGLE_RELEASE_DEADLINE_PLUS_DEPLOYMENT_EARLY_PREPARATION_HOLD \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /QUIESCED_EXACT_ATTACK_READINESS \+ SINGLE_RELEASE_DEADLINE_PROOF_WINDOW \/ PRE_RELEASE_EXACT_SEQUENCE_TARGET_AUDIO_PRIME \+ CONCURRENT_EXACT_EPOCH_AND_CAUSAL_ACCEPTANCE \+ IMMEDIATE_SCREENSHOT \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /EARLY_DEPLOYMENT_PREPARATION_PRESENTATION_QUIESCENCE \/ ARM_BEFORE_UNIT_ASSET_PROOF_AND_KEEP_THROUGH_FIXTURE_PLUS_FIRST_FRAME \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /0235da77bbc22d2b7b95740366b05089fc593884/u);
    assert.match(source, /650e07b9e74fd3913013c61d51a181dfe4d3426a/u);
    assert.match(source, /iteration 25/u);
    assert.match(source, /iteration 26/u);
  }
  for (const source of [r67, r67Handoff, projectState]) {
    assert.match(source, /QA_HARNESS_DIAGNOSTIC_OPERATION_OWNERSHIP_COLLISION \/ OUTER_FIRST_FRAME_OPERATION_ENCAPSULATED_SERIAL_INNER_OPERATIONS_AGAINST_SINGLE_SLOT_LIFECYCLE \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /EARLY_QUIESCENCE_WITH_SERIAL_DIAGNOSTIC_OWNERSHIP \/ PRESENTATION_OWNER_OUTSIDE_DIAGNOSTIC_WRAPPERS \+ FIVE_NON_NESTED_OPERATIONS_IN_EXISTING_ORDER \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /0235da77bbc22d2b7b95740366b05089fc593884/u);
    assert.match(source, /650e07b9e74fd3913013c61d51a181dfe4d3426a/u);
    assert.match(source, /iteration 25/u);
    assert.match(source, /iteration 26/u);
  }
  for (const source of [r68, r68Handoff, projectState]) {
    assert.match(source, /SOL_OWNED_SOURCE_NEGATIVE_SCOPE_ALIAS \/ R67_RUNDEPLOYMENTCASE_WIDE_REGEX_SPANNED_FIRST_FRAME_OPERATION_TO_LATER_CHECKPOINT_PRESENTATION_CALLBACK \/ REMEDIATION_LOCAL/u);
    assert.match(source, /EARLY_WINDOW_REGION_SCOPED_NON_NESTING_ASSERTION \/ BOUND_NEGATIVE_TO_FIRST_FRAME_QUIESCENCE_REGION \+ RETAIN_EXACT_DIRECT_OWNER_AND_FIVE_OPERATION_POSITIVES \/ REMEDIATION_LOCAL/u);
    assert.match(source, /0235da77bbc22d2b7b95740366b05089fc593884/u);
    assert.match(source, /650e07b9e74fd3913013c61d51a181dfe4d3426a/u);
    assert.match(source, /iteration 25/u);
    assert.match(source, /iteration 26/u);
  }
  for (const source of [r69, r69Handoff, projectState]) {
    assert.match(source, /SOL_OWNED_RELEASE_ROUTE_LITERAL_OMISSION \/ R68_ACTIVE_HANDOFF_RESUME_FROM_ABBREVIATED_FIXED_HEAD_ROUTE_WITHOUT_FINAL_PRODUCER_CHECKPOINT_TOKEN \/ REMEDIATION_LOCAL/u);
    assert.match(source, /CANONICAL_RELEASE_TAIL_LITERAL_RESTORATION \/ EXPAND_ACTIVE_HANDOFF_RESUME_FROM_WITH_SOL_FINAL_REVIEW_AND_FINAL_PRODUCER_CHECKPOINT \+ NO_RUNTIME_CHANGE \/ REMEDIATION_LOCAL/u);
    assert.match(source, /0235da77bbc22d2b7b95740366b05089fc593884/u);
    assert.match(source, /650e07b9e74fd3913013c61d51a181dfe4d3426a/u);
    assert.match(source, /iteration 25/u);
    assert.match(source, /iteration 26/u);
  }
  for (const source of [r70, r70Handoff, projectState]) {
    assert.match(source, /QA_HARNESS_AUDIO_CONTRACT_OWNER_ALIAS \/ PRESENTATION_INVENTORY_SEMANTIC_RANGER_CUE_USED_AS_PRODUCTION_RUNTIME_CUE \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /PRODUCTION_AUDIO_CUE_OWNER_RESOLUTION \/ ENEMY_COMBAT_CUE_FOR_ATTACK \+ WEAPON_CUE_FOR_UNIT \+ BOUNDED_READINESS_FAILURE_RECEIPT \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /0235da77bbc22d2b7b95740366b05089fc593884/u);
    assert.match(source, /650e07b9e74fd3913013c61d51a181dfe4d3426a/u);
    assert.match(source, /iteration 25/u);
    assert.match(source, /iteration 26/u);
  }
  for (const source of [r71, r71Handoff, projectState]) {
    assert.match(source, /QA_HARNESS_EXACT_WITNESS_MUTABILITY \/ FIRST_POST_RELEASE_ATTACK_RECEIPT_OVERWRITTEN_BY_LATER_FRAMES \+ FINAL_STATE_SOURCE_LIVENESS_INVALIDATES_COMPLETED_CAUSAL_PROOF \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /IMMUTABLE_FIRST_QUALIFYING_EXACT_WITNESS \/ FREEZE_ATTACK_IDENTITY_SEQUENCE_TARGET_SOURCE_LIVENESS_AND_PAGE_TIME \+ EVENT_TIME_DEADLINE_OWNERSHIP \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /0235da77bbc22d2b7b95740366b05089fc593884/u);
    assert.match(source, /650e07b9e74fd3913013c61d51a181dfe4d3426a/u);
    assert.match(source, /iteration 25/u);
    assert.match(source, /iteration 26/u);
  }
  for (const source of [r72, r72Handoff, projectState]) {
    assert.match(source, /QA_HARNESS_CROSS_UNIT_PAUSE_HANDOFF_GAP \/ PREVIOUS_FULLY_OUTSIDE_CHECKPOINT_LEAVES_PRODUCTION_PAUSED \+ NEXT_UNIT_FIRST_FRAME_QUIESCENCE_REQUIRES_LIVE_UNPAUSED_BATTLE \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /BOUNDED_CROSS_UNIT_LIVE_BATTLE_RESUME \/ RELEASE_PREVIOUS_SEMANTIC_PAUSE_ONCE \+ PROVE_LIVE_BATTLE_BEFORE_NEXT_UNIT_PRESENTATION_ARM \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /384bbc108e3aadcec134d842ed4c08f0d730fd29/u);
    assert.match(source, /a73c933f683a40cdf938aec316b477246cab484c/u);
    assert.match(source, /iteration 26/u);
    assert.match(source, /iteration 27/u);
  }
  for (const source of [r73, r73Handoff, projectState]) {
    assert.match(source, /SOL_OWNED_SOURCE_NEGATIVE_ASSIGNMENT_REGEX_ALIAS \/ R72_FORBIDDEN_DIRECT_STATE_ASSIGNMENT_REGEX_MATCHES_STRICT_EQUALITY_COMPARISONS \/ REMEDIATION_LOCAL/u);
    assert.match(source, /ASSIGNMENT_ONLY_NEGATIVE_BOUNDARY \/ REQUIRE_SINGLE_EQUALS_NOT_EQUALITY_OPERATOR \+ PRESERVE_R72_HARNESS_BYTES \/ REMEDIATION_LOCAL/u);
    assert.match(source, /384bbc108e3aadcec134d842ed4c08f0d730fd29/u);
    assert.match(source, /a73c933f683a40cdf938aec316b477246cab484c/u);
    assert.match(source, /iteration 26/u);
    assert.match(source, /iteration 27/u);
  }
  for (const source of [r74, r74Handoff]) {
    assert.match(source, /R72_R73_LOCAL_REMEDIATION_ACCEPTED \/ CROSS_UNIT_LIVE_RESUME \+ ASSIGNMENT_ONLY_NEGATIVE_BOUNDARY \/ LOCAL_GREEN/u);
    assert.match(source, /BOUNDED_CROSS_UNIT_LIVE_BATTLE_RESUME \+ ASSIGNMENT_ONLY_NEGATIVE_BOUNDARY \/ LOCAL_ACCEPTANCE_COMPLETE/u);
    assert.match(source, /384bbc108e3aadcec134d842ed4c08f0d730fd29/u);
    assert.match(source, /a73c933f683a40cdf938aec316b477246cab484c/u);
    assert.match(source, /iteration 26/u);
    assert.match(source, /iteration 27/u);
  }
  for (const source of [r75, r75Handoff]) {
    assert.match(source, /QA_HARNESS_PRE_RELEASE_WITNESS_CONSUMPTION \/ COMPLETED_HIDDEN_ATTACK_SELECTED_THEN_RELEASE_BASELINE_ADVANCED_TO_ITS_SEQUENCE \/ UNGUARANTEED_SECOND_EXACT_ATTACK_INSIDE_FIXED_VISIBLE_WINDOW \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /ATOMIC_UNCONSUMED_ATTACK_WINDUP_HANDOFF \/ EXACT RELEASE_ANCHOR_SELECTED_BEFORE_SEQUENCE_AND_CUE \+ VISIBLE_FIRST_QUALIFYING_ATTACK \/ PRODUCT_UNCHANGED \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /9c777d61c7863370d99c9112aa59365869b88a2b/u);
    assert.match(source, /ac3bb16aabfa28ee19a3dac2137f56a86b4cb9ee/u);
    assert.match(source, /iteration 27/u);
    assert.match(source, /iteration 28/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `14`/u);
  }
  for (const source of [r76, r76Handoff]) {
    assert.match(source, /QA_HARNESS_POST_COMMIT_TARGET_SOURCE_MISMATCH \/ EXACT_RELEASE_ANCHOR_SEQUENCE_AND_CUE_COMMITTED_BUT_MUTABLE_ATTACKER_TARGET_CLEARED_BEFORE_OBSERVER_READ \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /IMMUTABLE_RELEASE_ANCHOR_TARGET_RECOVERY \/ FIRST_BASELINE_PLUS_ONE_COMMIT_INSIDE_DERIVED_WINDUP_BOUND \+ SAME_SNAPSHOT_LIVE_TARGET \+ EXACT_CUE \/ PRODUCT_UNCHANGED \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /9c777d61c7863370d99c9112aa59365869b88a2b/u);
    assert.match(source, /ac3bb16aabfa28ee19a3dac2137f56a86b4cb9ee/u);
    assert.match(source, /iteration 27/u);
    assert.match(source, /iteration 28/u);
  ...
  for (const source of [r77, r77Handoff]) {
    assert.match(source, /R76_LOCAL_ORDERED_ACCEPTANCE_COMPLETE \/ IMMUTABLE_WINDUP_TARGET_RECOVERY_VALIDATED \/ LOCAL_GREEN/u);
    assert.match(source, /R76_LOCAL_ACCEPTANCE_READBACK \/ MATERIAL_ITERATION_27_TRANSPORT_READY \/ GOVERNANCE_ONLY/u);
    assert.match(source, /9c777d61c7863370d99c9112aa59365869b88a2b/u);
    assert.match(source, /ac3bb16aabfa28ee19a3dac2137f56a86b4cb9ee/u);
    assert.match(source, /iteration 27/u);
    assert.match(source, /iteration 28/u);
  ...
  for (const source of [r78, r78Handoff]) {
    assert.match(source, /INDEPENDENT_QA_CONTROL_FLOW_AND_PRESENTATION_LIFETIME_FAILURES \/ STAGE25_TERMINAL_HANDOFF_GAP \+ DEPLOYMENT_SETUP_ANON_PIPE_BACKPRESSURE \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /COHERENT_QA_HARNESS_LIFETIME_CLOSURE \/ MONOTONIC_CONTACT_FIRST_TERMINAL_HANDOFF \+ BATTLE_READINESS_TO_FIRST_FRAME_CONTINUOUS_PRESENTATION_QUIESCENCE \/ PRODUCT_UNCHANGED \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /6f1c243d940f308b349f23fcfc06282c8e826838/u);
    assert.match(source, /fb0cfd48b41700f6756671d58c467f4840dbac67/u);
    assert.match(source, /iteration 28/u);
    assert.match(source, /iteration 29/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `15`/u);
  }
  for (const source of [r79, r79Handoff]) {
    assert.match(source, /R78_LOCAL_COHERENT_ACCEPTANCE_COMPLETE \/ TERMINAL_HANDOFF_AND_SETUP_PRESENTATION_LIFETIME_VALIDATED \/ LOCAL_GREEN/u);
    assert.match(source, /R78_LOCAL_ACCEPTANCE_READBACK \/ MATERIAL_ITERATION_28_TRANSPORT_READY \/ GOVERNANCE_ONLY/u);
    assert.match(source, /6f1c243d940f308b349f23fcfc06282c8e826838/u);
    assert.match(source, /fb0cfd48b41700f6756671d58c467f4840dbac67/u);
    assert.match(source, /iteration 28/u);
    assert.match(source, /iteration 29/u);
  ...
  const currentOwners = [r98, activeHandoff, currentCursor].map((source) => source.match(/NEXT_OWNER`: `(SOL_DESIGN|SOL_REMEDIATION)`/u)?.[1] ?? "");
  ...
  assert.match(r63Handoff, /Section 79/u);
  assert.match(r63Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r64, /required Phase G job `97685910190`/u);
  assert.match(r64, /Artifact `9549339644`/u);
  assert.match(r64, /17 `anon_pipe_write`/u);
  assert.match(r64, /exactly these six tracked paths/u);
  assert.match(r64, /exactly the three WebKit battle-extra variants/u);
  assert.match(r64, /Arm immediately after the production battle mounts/u);
  assert.match(r64, /at least three new production render frames/u);
  assert.match(r64, /one fresh filtered ordered WebKit process/u);
  assert.match(r64, /High ambiguity: 0/u);
  assert.match(r64, /Medium ambiguity: 0/u);
  assert.match(r64Handoff, /Section 80/u);
  assert.match(r64Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r65, /first and only r64 local ordered WebKit process/u);
  assert.match(r65, /91\.797 seconds/u);
  assert.match(r65, /audioCueRequestBaseline: 128/u);
  assert.match(r65, /\.slice\(-128\)/u);
  assert.match(r65, /v100-phase-g-post-quiescence-proof\/v2/u);
  assert.match(r65, /audioCueRequestCutoffAt = performance\.now\(\)/u);
  assert.match(r65, /strictly greater than that cutoff/u);
  assert.match(r65, /one fresh filtered ordered WebKit process/u);
  assert.match(r65, /first and only r65 filtered ordered WebKit process/u);
  assert.match(r65, /Stage 6 WebKit 667x375, Stage 24 WebKit 736x414, and Stage 25 WebKit 932x430 exactly 3\/3/u);
  assert.match(r65, /audioCueRequestBaselineCount/u);
  assert.match(r65, /enemy-red-panther-commander-attack/u);
  assert.match(r65, /f2a0ac1b22ac2e49bf5a8c44b7712db05aa103efb9a5dc7b50be1c460aed85e2/u);
  assert.match(r65, /complete tests 1,197\/1,197/u);
  assert.match(r65, /R65_LOCAL_ACCEPTANCE_GREEN/u);
  assert.match(r65, /High ambiguity: 0/u);
  assert.match(r65, /Medium ambiguity: 0/u);
  assert.match(r65Handoff, /Section 81/u);
  assert.match(r65Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r65Handoff, /R65_LOCAL_ACCEPTANCE_GREEN/u);
  assert.match(r65Handoff, /r65-local-ordered-1/u);
  assert.match(r66, /[Aa]utomatic CI run `32817481135`/u);
  assert.match(r66, /required Phase G job `97712613534`/u);
  assert.match(r66, /artifact[^\n]+`9552439709`/u);
  assert.match(r66, /13 WebContent D-state samples/u);
  assert.match(r66, /12 `anon_pipe_write`/u);
  assert.match(r66, /deployment job `97714727514`/u);
  assert.match(r66, /ID `9553171122`/u);
  assert.match(r66, /58d29650b9019ce5dca62ca888b57822f35bc851dfdb1e6b951d0d85092da6ec/u);
  assert.match(r66, /49 WebContent D-state samples/u);
  assert.match(r66, /41 `anon_pipe_write`/u);
  assert.match(r66, /QA_HARNESS_POST_RELEASE_PRESENTATION_WINDOW_UNBOUNDED/u);
  assert.match(r66, /QA_HARNESS_DEPLOYMENT_PREPARATION_PRESENTATION_WINDOW_UNBOUNDED/u);
  assert.match(r66, /v100-phase-g-pre-release-readiness\/v1/u);
  assert.match(r66, /v100-phase-g-post-quiescence-proof\/v3/u);
  assert.match(r66, /visibleProofDeadlineAt/u);
  assert.match(r66, /12,000 ms normally and 4,800 ms for Stage 24/u);
  assert.match(r66, /exactly these eight tracked paths/u);
  assert.match(r66, /run-v099-deployment-units-bounded\.mjs/u);
  assert.match(r66, /High ambiguity: 0/u);
  assert.match(r66, /Medium ambiguity: 0/u);
  assert.match(r66Handoff, /Section 82/u);
  assert.match(r66Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r66Handoff, /R66_COHERENT_WEBKIT_PRESENTATION_LIFETIME_REMEDIATION_READY/u);
  assert.match(r67, /first and only fresh r66 WebKit 1280x720 deployment process/u);
  assert.match(r67, /zombieee-r66-deployment-01a00a4c-fcfa-7f61/u);
  assert.match(r67, /34f1e5b28cdfc52c3fc5f89cf288af09026f2eee09e74b7e0fed0b97ce6078af/u);
  assert.match(r67, /deployment telemetry operation mismatch none\/deployment\/first-frame-queue-readback/u);
  assert.match(r67, /createDeploymentLifecycleRecorder` owns only one `currentOperation` slot/u);
  assert.match(r67, /exactly five non-nested diagnostic operations/u);
  assert.match(r67, /a14b0938da749dfdfc9d4b2ffe49c8347d40371b053b735b1181bdf246bc7af3/u);
  assert.match(r67, /acb64e9b36e2721945865fa3bf4e5a1fcbcc7341bbb8b1697e3600587a5ab461/u);
  assert.match(r67, /High ambiguity: 0/u);
  assert.match(r67, /Medium ambiguity: 0/u);
  assert.match(r67Handoff, /Section 83/u);
  assert.match(r67Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r67Handoff, /R67_SERIAL_DEPLOYMENT_DIAGNOSTIC_OWNERSHIP_READY/u);
  assert.match(r68, /first and only r67 runtime-evidence source run/u);
  assert.match(r68, /2 pass \/ 1 fail/u);
  assert.match(r68, /firstFrameQuiescenceRegion/u);
  assert.match(r68, /153,348 bytes/u);
  assert.match(r68, /a8a5dfc19a2687aab20632e155558f2789b65b8189c208823d20f96cbf64e3fe/u);
  assert.match(r68, /High ambiguity: 0/u);
  assert.match(r68, /Medium ambiguity: 0/u);
  assert.match(r68Handoff, /Section 84/u);
  assert.match(r68Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r68Handoff, /R68_REGION_SCOPED_SOURCE_ASSERTION_READY/u);
  assert.match(r69, /first and only r68 Design Lock run/u);
  assert.match(r69, /18 pass \/ 1 fail/u);
  assert.match(r69, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  assert.match(r69, /High ambiguity: 0/u);
  assert.match(r69, /Medium ambiguity: 0/u);
  assert.match(r69Handoff, /Section 85/u);
  assert.match(r69Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r69Handoff, /R69_RELEASE_TAIL_AND_REGION_ASSERTION_READY/u);
  assert.match(r70, /first and only r69 ordered process/u);
  assert.match(r70, /r69-local-ordered-1/u);
  assert.match(r70, /81,893 ms/u);
  assert.match(r70, /88a39e2133625f25f1603eba66a4f1d8b32723113fda1f541771eadc960408a9/u);
  assert.match(r70, /b4abf96386b0931954527b17414e79eda92427e0c7671820932be5b990c9079d/u);
  assert.match(r70, /weapon-ranger-attack/u);
  assert.match(r70, /weapon-rifle/u);
  assert.match(r70, /enemy-spitter-attack/u);
  assert.match(r70, /enemyCombatCueFor/u);
  assert.match(r70, /weaponCueForUnit/u);
  assert.match(r70, /preReleaseReadiness/u);
  assert.match(r70, /at most four actors/u);
  assert.match(r70, /at most 16/u);
  assert.match(r70, /do not run the deployment browser process again/u);
  assert.match(r70, /High ambiguity: 0/u);
  assert.match(r70, /Medium ambiguity: 0/u);
  assert.match(r70Handoff, /Section 86/u);
  assert.match(r70Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r70Handoff, /R70_PRODUCTION_AUDIO_CUE_OWNER_REMEDIATION_READY/u);
  assert.match(r71, /first and only r70 ordered process/u);
  assert.match(r71, /r70-local-ordered-1/u);
  assert.match(r71, /eighteen milliseconds/u);
  assert.match(r71, /9f25a9ab0f4e8f44fc92480c891c62721e8b422e32dc0a3aaa532b6cd8d2d737/u);
  assert.match(r71, /829be2c2a60c7c6c627547357bface9fbbfaa0c2068795075fb497647bceeeb5/u);
  assert.match(r71, /sourceAliveAtObservation/u);
  assert.match(r71, /observedAtPageTime/u);
  assert.match(r71, /audioObservedAtPageTime/u);
  assert.match(r71, /proofCompletedAtPageTime/u);
  assert.match(r71, /final collector `pageNow` as diagnostics only/u);
  assert.match(r71, /screenshot receipt/u);
  assert.match(r71, /exactly 12 tests/u);
  assert.match(r71, /do not run the deployment browser process again/u);
  assert.match(r71, /first and only r71 filtered ordered WebKit process/u);
  assert.match(r71, /r71-local-ordered-1/u);
  assert.match(r71, /Stage 6 WebKit 667x375, Stage 24 WebKit 736x414, and Stage 25 WebKit 932x430 exactly 3\/3/u);
  assert.match(r71, /e6540b092c1bc10b6410cef3f149e35c7239616a49fcc909243b814e9c25cc1b/u);
  assert.match(r71, /8394fcd42057cc21630b7cc9510506d7a0ba182be94e55cc79338ccdfc614a1c/u);
  assert.match(r71, /75cffb801a9d89c52e5bf98a94387891f6218f277353259f542471a95af0d0a0/u);
  assert.match(r71, /71411099f20bb8ef1c6f3a06b2d95050835ea7e086a98ce3642b822cad983b96/u);
  assert.match(r71, /9e3cce2e9f7bcd5572eeb1b88715c9ca65228b570e0045ac525b04e817768262/u);
  assert.match(r71, /f2b52db3cf1410b2558ea95f08ee67552a9afcbf8691c289baff99e3f43c3a9d/u);
  assert.match(r71, /complete tests 1,197\/1,197/u);
  assert.match(r71, /R71_LOCAL_ACCEPTANCE_GREEN/u);
  assert.match(r71, /High ambiguity: 0/u);
  assert.match(r71, /Medium ambiguity: 0/u);
  assert.match(r71Handoff, /Section 87/u);
  assert.match(r71Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r71Handoff, /R71_LOCAL_ACCEPTANCE_GREEN/u);
  assert.match(r71Handoff, /r71-local-ordered-1/u);
  assert.match(r72, /Automatic focused CI run `32926247795`/u);
  assert.match(r72, /PR Verify job `98049635346`/u);
  assert.match(r72, /artifact is ID `9591755756`/u);
  assert.match(r72, /4118c47ae935386d02ea3646deda44c52a53bbe4bb9d9e4e316a3514fa656ee1/u);
  assert.match(r72, /All six required Chromium viewports/u);
  assert.match(r72, /36\/36 checkpoint PNGs/u);
  assert.match(r72, /all 42 unique\/disk-hash-verified artifact records/u);
  assert.match(r72, /zero cooperative trace samples/u);
  assert.match(r72, /resumeDeploymentBattleForNextUnit/u);
  assert.match(r72, /deployment\/cross-unit-live-resume/u);
  assert.match(r72, /v100-deployment-cross-unit-live-resume\/v1/u);
  assert.match(r72, /deploymentUnits\.length - 1/u);
  assert.match(r72, /one existing 100 ms host turn/u);
  assert.match(r72, /exactly six tracked paths/u);
  assert.match(r72, /9e3cce2e9f7bcd5572eeb1b88715c9ca65228b570e0045ac525b04e817768262/u);
  assert.match(r72, /f2b52db3cf1410b2558ea95f08ee67552a9afcbf8691c289baff99e3f43c3a9d/u);
  assert.match(r72, /Chromium 667x375 `deployment` process using all eight canonical units in one page/u);
  assert.match(r72, /High ambiguity: 0/u);
  assert.match(r72, /Medium ambiguity: 0/u);
  assert.match(r72Handoff, /Section 88/u);
  assert.match(r72Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r72Handoff, /R72_CROSS_UNIT_LIVE_RESUME_REMEDIATION_READY/u);
  assert.match(r72Handoff, /resumeDeploymentBattleForNextUnit/u);
  assert.match(r73, /first and only runtime-evidence source run completed 2\/3/u);
  assert.match(r73, /160,696 bytes/u);
  assert.match(r73, /cca517d48097f1cf041738991f05716df98ed2e77fa4344b55bef24004352f85/u);
  assert.ok(r73.includes("\\.(?:paused|running|over)\\s*=(?!=)"));
  assert.match(r73, /High ambiguity: 0/u);
  assert.match(r73, /Medium ambiguity: 0/u);
  assert.match(r73Handoff, /Section 89/u);
  assert.match(r73Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r73Handoff, /R73_ASSIGNMENT_ONLY_SOURCE_REMEDIATION_READY/u);
  assert.match(r74, /first and only fresh Chromium 667x375 same-page deployment process/u);
  assert.match(r74, /8\/8 checkpoints|48\/48 checkpoints/u);
  assert.match(r74, /exactly seven schema-valid/u);
  assert.match(r74, /a77f504d390d7f37d356d0fa5849818ef2f11e39c2039ff6273e94ff4ef3f002/u);
  assert.match(r74, /full 1,197\/1,197/u);
  assert.match(r74, /High ambiguity: 0/u);
  assert.match(r74, /Medium ambiguity: 0/u);
  assert.match(r74Handoff, /Section 90/u);
  assert.match(r74Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r74Handoff, /R74_LOCAL_ACCEPTANCE_GREEN/u);
  assert.match(r75, /32930044262/u);
  assert.match(r75, /98063712912/u);
  assert.match(r75, /9593380067/u);
  assert.match(r75, /f2189be0146199326b2f640ece2fe7377955519c93604ed3b7b15bd466e6f329/u);
  assert.match(r75, /57\/57 valid samples over 10,997 ms/u);
  assert.match(r75, /about 1\.1 seconds later/u);
  assert.match(r75, /change exactly six tracked paths/u);
  assert.match(r75, /cca517d48097f1cf041738991f05716df98ed2e77fa4344b55bef24004352f85/u);
  assert.match(r75, /d7a5cc7543143f7d0a8903b2750b5c0f317491497eadc57f97cadd59c540bdce/u);
  assert.match(r75, /actorSpecs\[0\]` is the release anchor/u);
  assert.match(r75, /v100-phase-g-pre-release-readiness\/v2/u);
  assert.match(r75, /v100-phase-g-post-quiescence-proof\/v4/u);
  assert.match(r75, /observed 0-120 ms ago/u);
  assert.match(r75, /snapshot age plus 80 ms/u);
  assert.match(r75, /same release-anchor fighter is still alive\/ready\/in the same unconsumed windup/u);
  assert.match(r75, /Stage 6 the human ranger must still complete/u);
  assert.match(r75, /Phase G checkpoint tests exactly 12\/12/u);
  assert.match(r75, /one fresh filtered ordered WebKit Phase G process/u);
  assert.match(r75, /High ambiguity: 0/u);
  assert.match(r75, /Medium ambiguity: 0/u);
  assert.match(r75Handoff, /Section 91/u);
  assert.match(r75Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r75Handoff, /R75_ATOMIC_UNCONSUMED_WINDUP_HANDOFF_REMEDIATION_READY/u);
  assert.match(r76, /r75-local-ordered-1/u);
  assert.match(r76, /3f180e6b4a21b8822a2c24eb79438fdbd11f9936b7b0c1371ce75dc8df16c8d7/u);
  assert.match(r76, /adba82d2e644513493f750457efd8ea1b49d513b084b621b4a11b77b88d626fd/u);
  assert.match(r76, /page time 46,756 \/ battle time 44\.0666/u);
  assert.match(r76, /page time 47,021 \/ battle time 44\.35/u);
  assert.match(r76, /v100-phase-g-post-quiescence-proof\/v5/u);
  assert.match(r76, /targetEvidenceSource: release-anchor-bound-windup/u);
  assert.match(r76, /targetEvidenceSource: live-attacker-target/u);
  assert.match(r76, /baselineAttackSequence \+ 1/u);
  assert.match(r76, /max\(0\.8 seconds, attackWindupSeconds \+ 0\.5 seconds\)/u);
  assert.match(r76, /same production snapshot/u);
  assert.match(r76, /Supporting Stage 6 ranger[\s\S]*no anchor fallback/u);
  assert.match(r76, /High ambiguity: 0/u);
  assert.match(r76, /Medium ambiguity: 0/u);
  assert.match(r76Handoff, /Section 92/u);
  assert.match(r76Handoff, /R76_IMMUTABLE_WINDUP_TARGET_RECOVERY_READY/u);
  assert.match(r77, /r76-local-ordered-1/u);
  assert.match(r77, /18c0080b62c46cb4deef54e9aeb89b007c4465dd540ee0326397e21ee2d41765/u);
  assert.match(r77, /a127605de03f2c065e9f366be42e4c5ed03991c2de442477baeb4092c3f49018/u);
  assert.match(r77, /947c7dc482b64a730d0e2995404687f3d3155de1f5eed654454c73fc0dbae5b7/u);
  assert.match(r77, /5da948ccf8ae79ab9329d0a7e57a72a1ac790a4beb1b4bae563bc9ca407b40ac/u);
  assert.match(r77, /targetEvidenceSource: live-attacker-target/u);
  assert.match(r77, /not selected at runtime and is not claimed as runtime evidence/u);
  assert.match(r77, /1,197\/1,197/u);
  assert.match(r77, /High ambiguity: 0/u);
  assert.match(r77, /Medium ambiguity: 0/u);
  assert.match(r77Handoff, /Section 93/u);
  assert.match(r77Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r77Handoff, /R77_LOCAL_ACCEPTANCE_GREEN/u);
  assert.match(r78Handoff, /Section 94/u);
  assert.match(r78Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r78Handoff, /R78_COHERENT_CONTACT_AND_DEPLOYMENT_LIFETIME_REMEDIATION_READY/u);
  assert.match(r78, /32936850890/u);
  assert.match(r78, /98083564943/u);
  assert.match(r78, /98086369208/u);
  assert.match(r78, /98090978050/u);
  assert.match(r78, /9595740041/u);
  assert.match(r78, /9596050441/u);
  assert.match(r78, /5c55d4bc8dd60937d99991b0b5a1893f18fa0ffaf86270dbc8f007a4b6c80b05/u);
  assert.match(r78, /0b675e5f405fb5b1b09b9af55ee67663a2f0695c6b58618f0ef67cfc08483ee3/u);
  assert.match(r78, /six accepted opening deployments/u);
  assert.match(r78, /after the fourth accepted deployment/u);
  assert.match(r78, /before each next deployment slot/u);
  assert.match(r78, /Historical target ownership alone must never satisfy branch \(b\)/u);
  assert.match(r78, /exactly fifteen/u);
  assert.match(r78, /QA_HARNESS_CONTACT_FIRST_TERMINAL_HANDOFF_GAP \/ HISTORICAL_EXACT_ATTACK_OBSERVED_BUT_OUTER_BOSS_FRONTLINE_LOOP_CONTINUED/u);
  assert.match(r78, /REMOTE_WEBKIT_DEPLOYMENT_SETUP_PRESENTATION_TRANSPORT_BACKPRESSURE \/ POST_READINESS_PRE_QUIESCENCE_ASSET_BOUNDARY_PAGE_EVALUATE \+ ANON_PIPE_WRITE_D_STATE \/ DESIGN_CHANGE_REQUIRED/u);
  assert.match(r78, /BATTLE_READINESS_TO_FIRST_FRAME_CONTINUOUS_PRESENTATION_QUIESCENCE \/ ARM_BEFORE_POST_READINESS_SETTLING_AND_ASSET_BOUNDARY \+ REUSE_SAME_ARM_THROUGH_FIRST_SEMANTIC_PAUSE \/ DESIGN_CHANGE_REQUIRED/u);
  for (const path of [
    "scripts/v100-phase-g-production-matrix.mjs",
    "tests/v100-phase-g-checkpoint.test.mjs",
    "scripts/v099-final-remediation-browser-smoke.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r78, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r78, /armedAtPageTime <= assetSetupBoundary\.boundaryAt/u);
  assert.match(r78, /one fresh bounded WebKit 736x414 canonical deployment process/u);
  assert.match(r78, /8\/8 units, 48\/48 checkpoints/u);
  assert.match(r78, /deployment runtime-evidence exactly 3\/3/u);
  assert.match(r78, /three separately named fresh Stage 25 WebKit 932x430 standalone controls/u);
  assert.match(r78, /High ambiguity: 0/u);
  assert.match(r78, /Medium ambiguity: 0/u);
  assert.match(r79Handoff, /Section 95/u);
  assert.match(r79Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r79Handoff, /R79_LOCAL_ACCEPTANCE_GREEN/u);
  assert.match(r79, /outputs\/r78-deployment-736x414-01a00a4c-fcfa-7f61-run1/u);
  assert.match(r79, /6e25b87412fe434e964b96a42c92de2e1572d6347300e60465eda84e3c6be778/u);
  assert.match(r79, /2f04194edeebc2e2cabcb31f96aeeb2021ce18df81b0455e17c0c0e1d1b1181b/u);
  assert.match(r79, /bad7d9b0781482f4e7fcf3db767719a96a181e4e1311143e719062340f41e427/u);
  assert.match(r79, /9f5142e636b7a1066f12024f99724a98569c154e07118fc7c0cf9f2217133620/u);
  assert.match(r79, /41dd4388e6e03d7ff3cf2c625e58108c7fd0bdd973ee267d4a953c12ff01859a/u);
  assert.match(r79, /1797deb90ccf82f9079eb9367dc8b1b215b265cc9680493515815c27759b959f/u);
  assert.match(r79, /a004967bf346aae48e6e3864a0f9c481174c4c5233934d98c51589f899671cf2/u);
  assert.match(r79, /e547bd0cdbc7aff4672e1388dcb2cd5de3916b98bfb692e4f4e050c455705fcb/u);
  assert.match(r79, /f16a3c206886aa65095c9731ffc6be31a84468df24a486a15cebe439c6f47216/u);
  assert.match(r79, /8\/8 units, 48\/48 checkpoints/u);
  assert.match(r79, /56\/56 unique disk-hash-verified artifacts/u);
  assert.match(r79, /Stage 25 WebKit 932x430 standalone processes then passed 1\/1 each/u);
  assert.match(r79, /r78-local-ordered-1/u);
  assert.match(r79, /passed Stage 6 WebKit 667x375, Stage 24 WebKit 736x414, and Stage 25 WebKit 932x430 exactly 3\/3/u);
  assert.match(r79, /1,197\/1,197/u);
  assert.match(r79, /High ambiguity: 0/u);
  assert.match(r79, /Medium ambiguity: 0/u);
  assert.match(r80Handoff, /Section 96/u);
  assert.match(r80Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r80Handoff, /R80_BOUNDED_QA_EVIDENCE_TRANSACTION_LIFETIME_REMEDIATION_READY/u);
  for (const source of [r80, r80Handoff]) {
    ...
    assert.match(source, /32945140417/u);
    assert.match(source, /98108463685/u);
    assert.match(source, /98112126403/u);
    assert.match(source, /98113667848/u);
    assert.match(source, /INDEPENDENT_QA_TRANSPORT_LIFETIME_FAILURES \/ PHASE_G_POST_CONVERGENCE_READBACK \+ HOSTED_MUTABLE_SCREENSHOT \+ DEPLOYMENT_ATOMIC_SIX_PASS_AUDIT \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /BOUNDED_QA_EVIDENCE_TRANSACTION_LIFETIME \/ LAST_ATOMIC_CAUSAL_RECEIPT \+ MUTABLE_SCREENSHOT_PRESENTATION_QUIESCENCE \+ SIX_STEP_PIXEL_AUDIT_SESSION \/ PRODUCT_UNCHANGED \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
  }
  for (const path of [
    "scripts/v100-phase-g-production-matrix.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r80, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r80, /ten valid samples from eleven attempts/u);
  assert.match(r80, /proof was already green/u);
  assert.match(r80, /remove the redundant final `page\.evaluate`/u);
  assert.match(r80, /issue no further page sample/u);
  assert.match(r80, /v100-visual-integrity-screenshot-quiescence\/v1/u);
  assert.match(r80, /no greater than 10,000 ms/u);
  assert.match(r80, /at least three restored production render frames/u);
  assert.match(r80, /`typeof OffscreenCanvas` is `undefined`/u);
  assert.match(r80, /v100-fighter-unit-layer-audit-session\/v1/u);
  assert.match(r80, /exactly the next one pass/u);
  assert.match(r80, /all six steps/u);
  assert.match(r80, /per-transaction ceiling no greater than 2,000 ms/u);
  assert.match(r80, /one fresh complete Hosted visual-integrity WebKit process/u);
  assert.match(r80, /one fresh bounded WebKit 844x340 canonical deployment process/u);
  assert.match(r80, /material-iteration-29/u);
  assert.match(r80, /High ambiguity: 0/u);
  assert.match(r80, /Medium ambiguity: 0/u);
  ...
  for (const source of [r82, r82Handoff]) {
    ...
    assert.match(source, /R81_LOCAL_ACCEPTANCE_READBACK \/ MATERIAL_ITERATION_29_TRANSPORT_READY \/ GOVERNANCE_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 29|material-iteration-29/u);
    assert.match(source, /workflow-only iteration 30/u);
  }
  for (const path of [
    "scripts/v100-phase-g-production-matrix.mjs",
    "tests/v100-phase-g-checkpoint.test.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r81, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r81, /fb41bcbcbdd9fda9c876307fb7c382c4412618180e44badf4bb1a443ff175040/u);
  assert.match(r81, /cbc8e9c6c915608e4f74f8c55eeb10a7df03f7383c45bfea913298ead45c5e6a/u);
  assert.match(r81, /cached snapshot observed 35 ms earlier/u);
  assert.match(r81, /one and only one direct bridge combat-snapshot read/u);
  assert.match(r81, /zero cached observer snapshot reads/u);
  assert.match(r81, /sameTaskSnapshotReadCount: 1/u);
  assert.match(r81, /cachedObserverSnapshotUsedForHandoff: false/u);
  assert.match(r81, /three new separately named fresh Stage 25 WebKit 932x430 standalone controls/u);
  assert.match(r81, /High ambiguity: 0/u);
  assert.match(r81, /Medium ambiguity: 0/u);
  for (const hash of [
    "06daa47925b76eb09ed52af06cfff6199892b1ad820a97137f656764d40a2626",
    "8513358360b2af1c2951baa3d42de071c825fdce3e8cd95ee41d2b73847c3551",
    "17061faf0c07f688d8cc4924a158349dab8fccc339cb817b0069fdd0980d29a2",
    "dbec98d88c59cde96c39b7b2b6b42671549b4134a5af15ae77df41f4302a5960",
    "5ab71f9445496d97632bf3a1f145918b51f632c8f413916ff5ff239656eae7a6",
    "86536210c5f3dadb34f251b8dc809ed7a4d074d5d011b12356ab6cb5876ae910",
    "47382a4138e982c52cb9986218719abfb02f0d94cbf896cd812c8854501d9f61",
    "4b1ed7996b1d05c0637dc95816bb492cd772aa226dfa593773bc305b80df68e0",
  ]) assert.match(r82, new RegExp(hash, "u"));
  assert.match(r82, /Stage 25 WebKit 932x430 standalone processes all passed on their first attempt/u);
  assert.match(r82, /ordered Stage 6 -> 24 -> 25 WebKit process passed 3\/3/u);
  assert.match(r82, /all 45 mutable mission screenshots/u);
  assert.match(r82, /48\/48 checkpoints/u);
  assert.match(r82, /complete suite passed 1,197\/1,197/u);
  ...
  assert.match(r89Handoff, /Section 105/u);
  assert.match(r90Handoff, /Section 106/u);
  assert.match(r96Handoff, /Section 112/u);
  assert.match(r98Handoff, /Section 114/u);
  assert.match(r99Handoff, /Section 115/u);
  assert.match(r99Handoff, /NO ACTIVE LUNA HANDOFF/u);
  assert.match(r99Handoff, /R99_ACCEPTED_PROOF_SCREENSHOT_ORDER_PACKET_ACTIVE/u);
  assert.match(r100Handoff, /Section 116/u);
  assert.match(r100Handoff, /R100_PAGE_OWNED_PHASE_G_EVIDENCE_STATE_MACHINE_PACKET_ACTIVE/u);
  assert.match(r101Handoff, /Section 117/u);
  assert.match(r101Handoff, /R101_STRICT_V6_RECEIPT_DOMAIN_ACTIVE/u);
  assert.match(r102Handoff, /Section 118/u);
  assert.match(r102Handoff, /R102_SHARED_V6_TEST_CONTRACT_ACTIVE/u);
  assert.match(r103Handoff, /Section 119/u);
  assert.match(r104Handoff, /Section 120/u);
  assert.match(r104Handoff, /R104_SINGLE_SOURCE_CAUSAL_EVENT_MACHINE_ACTIVE/u);
  assert.match(activeHandoff, /Section 121/u);
  assert.match(activeHandoff, /NO ACTIVE LUNA HANDOFF/u);
  ...
  for (const source of [r85, r85Handoff]) {
    assert.match(source, /e8f5fb152acca9124fceead899734c5d368053ba/u);
    assert.match(source, /f58ccc5bc0e8a070cf002f13689e19c02920fe0b/u);
  ...
  for (const source of [r86, r86Handoff]) {
    assert.match(source, /e256d5616eec58f44b62a5098223eaa45a6e70b6/u);
    assert.match(source, /d6bc566912a9ad98bc99ed15f767cfe3fe3923ac/u);
    assert.match(source, /TRANSPORT_AUTHENTICATION_PRECONDITION \/ SANDBOX_HTTPS_SCHANNEL_SEC_E_NO_CREDENTIALS_BEFORE_REF_UPDATE \/ REMEDIATION_LOCAL/u);
    assert.match(source, /EXISTING_COMMIT_PRESERVATION \+ HOST_CREDENTIAL_CONTEXT_ONE_SHOT_NON_FORCE_PUSH \/ DIRECT_GITHUB_REMOTE \+ NO_SECRET_OUTPUT \/ GOVERNANCE_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /workflow-only iteration 31/u);
  }
  assert.match(r86, /before any remote ref update/u);
  assert.match(r86, /Do not amend, squash, rebase, reset, recommit, or rebuild/u);
  assert.match(r86, /one explicitly approved host credential context/u);
  assert.match(r86, /never print, query, copy, persist, or log a token\/password/u);
  assert.match(r86, /do not use `--force`, lease force, retry, or a second transport mechanism/u);
  assert.match(r86, /High ambiguity: 0/u);
  assert.match(r86, /Medium ambiguity: 0/u);
  for (const source of [r87, r87Handoff]) {
    assert.match(source, /e256d5616eec58f44b62a5098223eaa45a6e70b6/u);
    assert.match(source, /SOL_OWNED_ACTIVE_HANDOFF_RELEASE_TAIL_TOKEN_OMISSION \/ R86_SECTION95_LACKS_FINAL_PRODUCER_CHECKPOINT_LITERAL \/ REMEDIATION_LOCAL/u);
    assert.match(source, /ACTIVE_HANDOFF_RELEASE_TAIL_LITERAL_CLOSURE \/ SOL_FINAL_REVIEW \+ ONE_FINAL_PRODUCER_CHECKPOINT_SENTENCE \/ GOVERNANCE_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /workflow-only iteration 31/u);
  }
  assert.match(r87, /18 pass \/ 1 fail/u);
  assert.match(r87, /No canonical suite, staging, governance commit, host transport, remote CI/u);
  assert.match(activeHandoff, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  assert.match(activeHandoff, /SOL_FINAL_REVIEW/u);
  assert.match(r87, /High ambiguity: 0/u);
  assert.match(r87, /Medium ambiguity: 0/u);
  for (const source of [r88, r88Handoff]) {
    assert.match(source, /e1ba3677b62520fe5d7dd829999e61586b484c71/u);
    assert.match(source, /5e7f0523db939be39a71468a279a8249fd420c74/u);
    assert.match(source, /HOST_CREDENTIAL_REPOSITORY_OWNERSHIP_PRECONDITION \/ EXACT_ISOLATED_WORKTREE_OWNED_BY_CODEXSANDBOXONLINE \+ HOST_USER_OKAITO_REJECTED_BEFORE_NETWORK \/ REMEDIATION_LOCAL/u);
    assert.match(source, /COMMAND_SCOPED_EXACT_SAFE_DIRECTORY \+ EXISTING_COMMIT_CHAIN_PRESERVATION \/ NO_PERSISTENT_CONFIG \+ ONE_HOST_NORMAL_NON_FORCE_PUSH \/ GOVERNANCE_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /workflow-only iteration 31/u);
  }
  assert.match(r88, /before network authentication or remote ref update/u);
  assert.match(r88, /-c safe\.directory=C:\/Users\/okait\/Documents\/Codex\/2026-07-11\/new-chat\/_isolated\/v100-sol-r9-3a40/u);
  assert.match(r88, /No `--global`, `--system`, `--local`, wildcard trust/u);
  assert.match(r88, /Do not repeat accepted r84 Stage 3, HUD, deployment, content, complete-test, lint, or build gates/u);
  assert.match(activeHandoff, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  assert.match(activeHandoff, /SOL_FINAL_REVIEW/u);
  assert.match(r88, /High ambiguity: 0/u);
  assert.match(r88, /Medium ambiguity: 0/u);
  for (const source of [r89, r89Handoff]) {
    assert.match(source, /ba31c9e8e6b1c44e0a4376edcaedb3c30e6010c0/u);
    assert.match(source, /6e820e06d069285c69bfb874b193a3c2e1c33e8b/u);
    assert.match(source, /QA_HARNESS_REMAINING_WEBKIT_EVIDENCE_TRANSACTION_OWNERSHIP \/ STAGE25_MAX_CATCH_UP_FRAME_OMISSION \+ HOSTED_CROSS_STATE_OWNER_GAP \+ DEPLOYMENT_RECEIPT_TO_AUDIT_GAP \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /FINITE_ATOMIC_EVIDENCE_HANDOFF_PACKET \/ SCHEDULER_DERIVED_COMMIT_TOLERANCE \+ MUTABLE_STATE_RENDER_TO_OWNER_HANDOFF \+ CHECKPOINT_RECEIPT_TO_PREBOUND_AUDIT_SESSION \/ PRODUCT_UNCHANGED \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 31|material-iteration-31/u);
    assert.match(source, /workflow-only iteration 32/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  for (const path of [
    "scripts/v100-phase-g-production-matrix.mjs",
    "tests/v100-phase-g-checkpoint.test.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r89, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  for (const token of [
    "32971539028",
    "98190808503",
    "98191589350",
    ...
    "9608611517",
    "9608629248",
    "da04c6f59bf227ea07fdf1163c5356f4cc926f7d696271fac52fef64b616a718",
    "4bdd87e804eaec19127d0346edc3cc02a9df68219ddd3e6044ab67ad09bf23b1",
    "f6787acd308a0d9c81db7c820d544d96f9ddabe4cc8ea0c1eb2146645ef9f5081",
  ]) assert.match(r89, new RegExp(token, "u"));
  assert.match(r89, /RUNTIME_SIMULATION_STEP_SECONDS/u);
  assert.match(r89, /RUNTIME_MAX_CATCH_UP_STEPS/u);
  assert.match(r89, /Object\.isFrozen/u);
  assert.match(r89, /v100-deployment-checkpoint-audit-handoff\/v1/u);
  assert.match(r89, /ready 3\/3, fault 35\/35/u);
  assert.match(r89, /SAME_GATE_REPEAT_COUNT`: `18`/u);
  assert.match(r89, /High ambiguity: 0/u);
  assert.match(r89, /Medium ambiguity: 0/u);
  for (const source of [r90, r90Handoff]) {
    assert.match(source, /ba31c9e8e6b1c44e0a4376edcaedb3c30e6010c0/u);
    assert.match(source, /6e820e06d069285c69bfb874b193a3c2e1c33e8b/u);
    assert.match(source, /QA_HARNESS_HOSTED_RENDER_WINDOW_EXACT_EQUALITY_UNOBSERVABLE \/ INITIAL_MUTABLE_STATE_FINAL_CANVAS_GREEN_BUT_PAGE_OBSERVER_REACHED_DELTA_3_AFTER_MINIMUM_2 \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /BOUNDED_RENDER_OBSERVATION_WINDOW \/ SAME_PAGE_MINIMUM_RENDER_PLUS_ONE_FRAME_MAXIMUM_AND_ATOMIC_OWNER_REACQUISITION \/ PRODUCT_UNCHANGED \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 31|material-iteration-31/u);
    assert.match(source, /workflow-only iteration 32/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `18`/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  for (const token of [
    "1fadc92136a35c5ce5e353cbbc146d449fa995e6a7c95cf5f8e9802f0eb649d2",
    "3af9157e9ec5ec76410b1b0177d6c0bc51c4bbca29fda2c2e7d53e5e6606107a",
    "ed17eae30bc7be0a300c2af4f473a7266fa917c5bf676b4f9975ab73005ca29d",
    "b3c746e88a99d37ad5b0106162712d1141d6bec28d4796fa231c8209a408148e",
    "621f997b81403fa505de814e1dc0b4790a03e1317b085899a3c5b9da29db9b91",
    "cbbe51df8c1f4b8c0ebfde1bc30874b9199cd421628da9005f6cfb22b38349da",
  ]) assert.match(r90, new RegExp(token, "u"));
  assert.match(r90, /minimumRenderFrameDelta = 2/u);
  assert.match(r90, /minimumRenderFrameDelta = 3/u);
  assert.match(r90, /maximumRenderFrameDelta.*minimumRenderFrameDelta \+ 1/u);
  assert.match(r90, /at most eight lean observation records/u);
  assert.match(r90, /Do not repeat the accepted r89 Stage 25 standalone 3\/3 or ordered trio/u);
  assert.match(r90, /High ambiguity: 0/u);
  assert.match(r90, /Medium ambiguity: 0/u);
  for (const source of [r91, r91Handoff]) {
    assert.match(source, /ba31c9e8e6b1c44e0a4376edcaedb3c30e6010c0/u);
    assert.match(source, /6e820e06d069285c69bfb874b193a3c2e1c33e8b/u);
    assert.match(source, /QA_HARNESS_HOSTED_INITIAL_PREARM_STATE_PREDICATE_ALIAS \/ FIRST_MUTABLE_TRANSITION_REQUIRES_PREDECESSOR_RELEASED_ROUTE_OWNER_THAT_CANNOT_EXIST_BEFORE_GENERATION_1 \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /PHASE_TYPED_INITIAL_PREARM_AND_PREDECESSOR_RELEASED_PREDICATES \/ EXACT_2_AND_3_RENDER_COUNTS \+ SAME_PAGE_OWNER_ACQUISITION \/ PRODUCT_UNCHANGED \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 31|material-iteration-31/u);
    assert.match(source, /workflow-only iteration 32/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `18`/u);
  ...
  assert.match(r91, /initial-prearm/u);
  assert.match(r91, /predecessor-released/u);
  assert.match(r91, /requiredRenderFrameDelta = previousGeneration === null \? 2 : 3/u);
  assert.match(r91, /Do not implement a minimum\/maximum window or \+1 tolerance/u);
  assert.match(r91, /generation 0 -> 1/u);
  assert.match(r91, /High ambiguity: 0/u);
  assert.match(r91, /Medium ambiguity: 0/u);
  for (const source of [r92, r92Handoff]) {
    assert.match(source, /ba31c9e8e6b1c44e0a4376edcaedb3c30e6010c0/u);
    assert.match(source, /6e820e06d069285c69bfb874b193a3c2e1c33e8b/u);
    assert.match(source, /LOCAL_ACCEPTANCE_GREEN \/ R91_PHASE_TYPED_HOSTED_AND_DEPLOYMENT_PACKET_CLOSED \/ REMEDIATION_LOCAL/u);
    assert.match(source, /GOVERNANCE_READBACK_AND_IMMUTABLE_TRANSPORT \/ RUNTIME_BYTES_FROZEN \+ ONE_MATERIAL_ITERATION_31_COMMIT \/ GOVERNANCE_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 31|material-iteration-31/u);
    assert.match(source, /workflow-only iteration 32/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `18`/u);
  ...
  for (const token of [
    "b3c746e88a99d37ad5b0106162712d1141d6bec28d4796fa231c8209a408148e",
    "621f997b81403fa505de814e1dc0b4790a03e1317b085899a3c5b9da29db9b91",
    "e78cef388c18bb9b0d89a173095674a8430c7c2a987afd8feb84e11de658060d",
    "cbbe51df8c1f4b8c0ebfde1bc30874b9199cd421628da9005f6cfb22b38349da",
    "3a8bf4fe7848f044aa91ed3e20b67dbdf5d664285788c854eea62d0dc7f2dd61",
    "800c5ddb52ee116073be0a97b4f0824aba4229c97821b00232fa9738d229bc98",
    "5cf984669bbc5afe7c165be5d67af8cfadf25f1dafab25bcaf4510d70f48e1cc",
    "60fddf9ed48c81dac6a545fc8352160bbf386abe5edc81bd624b17c2a6427a95",
    "5139546496b8d57161ff150df550a72069d4a046346cc42a0406c3ac6ccbce90",
  ]) assert.match(r92, new RegExp(token, "u"));
  assert.match(r92, /ready 3\/3, fault 35\/35, final-canvas 15\/15, mutable 45\/45/u);
  assert.match(r92, /8\/8 units, 48\/48 checkpoints/u);
  assert.match(r92, /complete suite 1,197\/1,197/u);
  assert.match(r92, /Do not repeat Hosted, deployment, content, complete tests, lint, or build/u);
  assert.match(r92, /High ambiguity: 0/u);
  assert.match(r92, /Medium ambiguity: 0/u);
  for (const source of [r93, r93Handoff]) {
    assert.match(source, /124e7d0bad61ca3ee9bd92b3a08b137543b86b6c/u);
    assert.match(source, /b159a0ea5cb4258202a906469775ea9a0fc62363/u);
    assert.match(source, /32984411634/u);
    assert.match(source, /GITHUB_ACTIONS_DECLARED_MAJOR_OUTAGE \/ ZERO_STEP_HOSTED_RUNNER_ACQUISITION_CANCELLATION_WAVES_DURING_OFFICIAL_INCIDENT \/ REMEDIATION_LOCAL/u);
    assert.match(source, /GOVERNANCE_RECORDED_OFFICIAL_OUTAGE_RECOVERY_REENTRY \/ RUNTIME_BYTES_AND_ACCEPTED_LOCAL_EVIDENCE_PRESERVED \+ ONE_POST_RECOVERY_AUTOMATIC_FOCUSED_RUN \/ GOVERNANCE_ONLY/u);
    assert.match(source, /R93_OFFICIAL_ACTIONS_RECOVERY_CONFIRMED \/ EXACT_HEAD_REENTRY_READY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 31|material-iteration-31/u);
    assert.match(source, /workflow-only iteration 32/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `18`/u);
    assert.match(source, /HOSTED_RUNNER_ACQUISITION_FAILURE_COUNT`: `1`/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  for (const token of [
    "98228373814", "98228374065", "98228374039", "98228374037", "98228373906", "98228373999", "98228373925",
    "98234106347", "98234106318", "98234106358", "98234106349", "98234106333", "98234106309",
    "98230329966", "98234106674", "98234106486", "98262003402",
  ]) {
    assert.match(r93, new RegExp(token, "u"));
  }
  assert.match(r93, /The job was not acquired by Runner of type hosted even after multiple attempts/u);
  assert.match(r93, /zero workflow steps/u);
  assert.match(r93, /17\/17 jobs terminal/u);
  assert.match(r93, /Thirteen jobs completed `cancelled`/u);
  assert.match(r93, /Four dependency jobs completed `skipped`/u);
  assert.match(r93, /Incident with Actions/u);
  assert.match(r93, /Actions: Major Outage/u);
  assert.match(r93, /official incident to be resolved and Actions to report `Operational`/u);
  assert.match(r93, /All Systems Operational/u);
  assert.match(r93, /y1t7p9fzrlj2/u);
  assert.match(r93, /2026-08-26T18:01:30\.665Z/u);
  assert.match(r93, /2026-08-26T18:15:12Z/u);
  assert.match(r93, /Do not call a rerun-job, rerun-failed-jobs, workflow-dispatch, or manual UI action/u);
  assert.match(r93, /one normal non-amended four-path r93 governance child/u);
  assert.match(r93, /High ambiguity: 0/u);
  assert.match(r93, /Medium ambiguity: 0/u);
  for (const source of [r94, r94Handoff]) {
    assert.match(source, /124e7d0bad61ca3ee9bd92b3a08b137543b86b6c/u);
    assert.match(source, /b159a0ea5cb4258202a906469775ea9a0fc62363/u);
    assert.match(source, /a899376b2276b7299c7dab03f4967851ccfaeefc/u);
    assert.match(source, /7cbbaf50d6381d073b5ec74d65e4ac603837fc17/u);
    assert.match(source, /WINDOWS_SCHANNEL_TLS_CREDENTIAL_ACQUISITION_FAILURE \/ SYSTEM_HTTP_SSLBACKEND_SCHANNEL_RETURNS_SEC_E_NO_CREDENTIALS_BEFORE_GIT_HTTP_REF_UPDATE \/ REMEDIATION_LOCAL/u);
    assert.match(source, /COMMAND_SCOPED_OPENSSL_BACKEND \+ EXISTING_COMMIT_CHAIN_PRESERVATION \/ NO_PERSISTENT_CONFIG \+ ONE_NORMAL_NON_FORCE_PUSH \/ GOVERNANCE_ONLY/u);
    assert.match(source, /R94_COMMAND_SCOPED_OPENSSL_TRANSPORT_READY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 31|material-iteration-31/u);
    assert.match(source, /workflow-only iteration 32/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `18`/u);
    assert.match(source, /HOSTED_RUNNER_ACQUISITION_FAILURE_COUNT`: `1`/u);
    assert.match(source, /TRANSPORT_TLS_FAILURE_COUNT`: `1`/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  assert.match(r94, /git `2\.55\.0\.windows\.2`|Git `2\.55\.0\.windows\.2`/u);
  assert.match(r94, /http\.sslBackend=schannel/u);
  assert.match(r94, /http\.sslBackend=openssl/u);
  assert.match(r94, /credential\.interactive=never/u);
  assert.match(r94, /SEC_E_NO_CREDENTIALS/u);
  assert.match(r94, /read-only `ls-remote` succeeded/u);
  assert.match(r94, /No HTTP ref update occurred/u);
  assert.match(r94, /Do not write `--global`, `--system`, or `--local` config/u);
  assert.match(r94, /do not change[\s\S]*`http\.sslVerify`/u);
  assert.match(r94, /High ambiguity: 0/u);
  assert.match(r94, /Medium ambiguity: 0/u);
  for (const source of [r95, r95Handoff]) {
    assert.match(source, /124e7d0bad61ca3ee9bd92b3a08b137543b86b6c/u);
    assert.match(source, /b159a0ea5cb4258202a906469775ea9a0fc62363/u);
    assert.match(source, /7763763403546c00eeba4640dff83b18282b9ba8/u);
    assert.match(source, /8dc9503c0f398c91286481fce630b69bed448f29/u);
    assert.match(source, /GIT_HTTPS_AUTHENTICATED_WRITE_PATH_NONTERMINATION \/ COMMAND_SCOPED_OPENSSL_READ_PATH_GREEN_BUT_CREDENTIAL_HELPER_PUSH_REMAINED_SILENT_OVER_FIVE_MINUTES_BEFORE_REF_UPDATE \/ REMEDIATION_LOCAL/u);
    assert.match(source, /AUTHENTICATED_GITHUB_GIT_OBJECT_FAST_FORWARD \/ EXACT_LOCAL_R95_TREE_REMATERIALIZATION_FROM_REMOTE_R92_BASE \+ ONE_NON_FORCE_REF_UPDATE \/ GOVERNANCE_ONLY/u);
    assert.match(source, /R95_AUTHENTICATED_OBJECT_FAST_FORWARD_READY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 31|material-iteration-31/u);
    assert.match(source, /workflow-only iteration 32/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `18`/u);
    assert.match(source, /HOSTED_RUNNER_ACQUISITION_FAILURE_COUNT`: `1`/u);
    assert.match(source, /TRANSPORT_TLS_FAILURE_COUNT`: `1`/u);
    assert.match(source, /TRANSPORT_WRITE_NONTERMINATION_COUNT`: `1`/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  assert.match(r95, /more than five minutes/u);
  assert.match(r95, /Ctrl\+C/u);
  assert.match(r95, /identity is `SUSANO-OOO`/u);
  assert.match(r95, /permission is `admin`/u);
  assert.match(r95, /base64 transport/u);
  assert.match(r95, /git rev-parse HEAD:<path>/u);
  assert.match(r95, /mode `100644`, type `blob`/u);
  assert.match(r95, /docs: close post-outage transport re-entry/u);
  assert.match(r95, /force: false/u);
  assert.match(r95, /server-generated commit identity|GitHub commit API controls commit identity metadata/u);
  assert.match(r95, /Do not repeat Hosted, deployment, Phase G, content, complete tests, lint, or build/u);
  assert.match(r95, /High ambiguity: 0/u);
  assert.match(r95, /Medium ambiguity: 0/u);
  for (const source of [r96, r96Handoff]) {
    assert.match(source, /f8df99b8724964ee3e33c4a41d0bbe3e74ba6356/u);
    assert.match(source, /365099e0055536887c03324f6a609dd1de7bf810/u);
    assert.match(source, /33002403217/u);
    assert.match(source, /98295009478/u);
    assert.match(source, /98296302174/u);
    assert.match(source, /DUAL_QA_PRESENTATION_LIFETIME_GAPS \/ STAGE24_NON_CONTACT_FIRST_EXACT_ACTOR_SETUP_HANDOFF \+ STAGE3_PRE_DISPATCH_TO_FIRST_FINAL_CUT_OWNER \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /COHERENT_QA_PRESENTATION_OWNERSHIP_PACKET \/ GENERALIZED_EXACT_ACTOR_TERMINAL_HANDOFF \+ CONTINUOUS_STAGE3_FINAL_ENTRY_GUARD \/ QA_HARNESS_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 32|material-iteration-32/u);
    assert.match(source, /workflow-only iteration 33/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `19`/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  for (const token of [
    "9620322394",
    "9620435208",
    "7c672900ea234aa13e1c36be8a5606d08e60e6929e0dbdc0277efbfdbc0d9ac3",
    "9f00b945f1857dc49b42ef5eff67d38722e26528edec473fe50cac1ce714600f",
    "775fcb7c7bc907ec1e00cbcd953402ebc5b2697080e0918d937acbb4633cab29",
    "108a25addbb7a533d653006dea56d04d07192d96678ac05304926bd168c0118e",
    "b41129323c2828fcd5074b7d0aeb9fd6821dbecd3a5d8fe4679e9d80eb35821d",
    "fcb11c836e49adcb872353b0db7c80d6227e968b6d7121e1a7ebaa3542eb9ab0",
    "181bab547804bcd7b5a183c7bad60ed22ed9b4995d2dd9bbe7a7723dc5ba9f66",
    "16384b81d1196b5b0a6c6d449a8338919f4f9c978bd173ca763451e98dff3792",
    "312a905e9a0946999d1a2c6171a557a46626f1d8ff3f0e4ac2f849b52304bc4b",
    "7f852b403cf0b3ff5d4232654e9b5d61b389786a6b7dd7c7c7da5b90014f4abb",
  ]) assert.match(r96, new RegExp(token, "u"));
  for (const path of [
    "scripts/v100-phase-g-production-matrix.mjs",
    "tests/v100-phase-g-checkpoint.test.mjs",
    "scripts/p5-browser-smoke.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r96, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r96, /bossFrontlineTerminalHandoff/u);
  assert.match(r96, /Historical target evidence alone must never latch/u);
  assert.match(r96, /proofActorAttackObserved === true/u);
  assert.match(r96, /contactState\.hasLiveHumanTarget === true/u);
  assert.match(r96, /fresh, exact, unconsumed production windup/u);
  assert.match(r96, /p5-stage3-final-entry-presentation-guard\/v1/u);
  assert.match(r96, /MutationObserver/u);
  assert.match(r96, /adoptEntryGuard: true/u);
  assert.match(r96, /three separately named fresh processes/u);
  assert.match(r96, /9\/9 green|all nine captures green/u);
  assert.match(r96, /complete suite `1,197\/1,197`/u);
  assert.match(r96, /lint `0` errors \/ `12` existing warnings/u);
  assert.match(r96, /test: close r96 presentation ownership gaps/u);
  assert.match(r96, /authenticated-admin GitHub object fast-forward/u);
  assert.match(r96, /create exactly eight base64 blobs/u);
  assert.match(r96, /exactly one `force: false` ref update/u);
  assert.match(r96, /LOCAL_WINDOWS_WEBKIT_AUDIO_UNLOCK_CAPABILITY_INTERPOSITION \/ COMPARISON_ONLY/u);
  assert.match(r96, /No ninth path/u);
  assert.match(r96, /High ambiguity: 0/u);
  assert.match(r96, /Medium ambiguity: 0/u);
  for (const source of [r97, r97Handoff]) {
    assert.match(source, /1ca4572b3898fa6f43e9f94eed52ab69660052a0/u);
    assert.match(source, /315caa0a60745bc13e8f04c4b7e36bf1ac0e22de/u);
    assert.match(source, /33012551716/u);
    assert.match(source, /98327087386/u);
    assert.match(source, /98328484702/u);
    assert.match(source, /98329161546/u);
    assert.match(source, /THREE INDEPENDENT QA EVIDENCE-TRANSACTION GAPS \/ HOSTED SUCCESSOR SUPPRESSION SPLIT \+ DEPLOYMENT HEAVY AUDIT BEGIN IN READY POLL \+ PHASE_G EARLY SINGLE-SNAPSHOT WINDUP AND PARTIAL RESIDUAL RPC \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /COHERENT QA TRANSACTION-OWNERSHIP PACKET \/ SAME-PAGE SUCCESSOR SUPPRESSION \+ LEAN-THEN-BOUNDED PIXEL SESSION \+ DISTINCT-FRAME LATE-WINDUP CONTINUITY AND FULL-BUDGET-ONLY CAUSAL RPC \/ QA_HARNESS_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 33|material-iteration-33/u);
    assert.match(source, /workflow-only iteration 34/u);
    assert.match(source, /SAME_GATE_REPEAT_COUNT`: `20`/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  for (const token of [
    ...
    "9623971981",
    "9624041421",
    "1db308389236c827835bed06c920b4c08f98502ca19b8dc269a1442da0c646e3",
    "78bd261ad9631749d4db278a2b14e33ef1dacb052e4b8bf240c64c0dd2311f88",
    "6375c802e6306bf200aab421a9e1b202d50de922ee3c5ff9ca00858450d7ecf3",
    "afc221deccaf1df1c00f2299662edf2de961482ad065a25482a02d60eeb8f206",
    "da810917f08a69d2cbdee588950eb9c9d07444415d943f87462d262ad139d25b",
    "0d4b0871d4a2c1a307d704b107b0b64f5d928fd69eec8fb5bde4a18632e10865",
    "7103faa00b671fbfa084036dd4a27d6c38ead1c78fdd4758a114f7cf8f0a0681",
    "4f4c9153389ea1160461fc9ea5c5025cd073133c1b254c511acd2cb22707d14a",
    "e0d3b9d3e1797af9687915f1ae9367dadab2dbe27d82730892776e04980d55ca",
  ]) assert.match(r97, new RegExp(token, "u"));
  for (const path of [
    "scripts/v0995-visual-integrity-browser-smoke.mjs",
    "scripts/v099-final-remediation-browser-smoke.mjs",
    "scripts/v100-phase-g-production-matrix.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r97, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r97, /successorSuppression/u);
  assert.match(r97, /transitionTimeoutMs` of 2,000 ms/u);
  assert.match(r97, /v100-deployment-checkpoint-audit-ready\/v1/u);
  assert.match(r97, /beginFighterUnitLayerAuditSession/u);
  assert.match(r97, /RUNTIME_SIMULATION_STEP_SECONDS \* RUNTIME_MAX_CATCH_UP_STEPS/u);
  assert.match(r97, /polling: "raf"/u);
  assert.match(r97, /COMBAT_CAUSAL_PAGE_TRANSACTION_TIMEOUT_MS` of 2,000 ms/u);
  assert.match(r97, /test: close r97 evidence transaction boundaries/u);
  assert.match(r97, /exactly nine committed blobs/u);
  assert.match(r97, /exactly one `force: false` ref update/u);
  assert.match(r97, /No tenth path/u);
  assert.match(r97, /High ambiguity: 0/u);
  assert.match(r97, /Medium ambiguity: 0/u);
  for (const source of [r98, r98Handoff]) {
    assert.match(source, /955bf287fa305d3d940a56a15bdb203f062ca27c/u);
    assert.match(source, /4b678ec0e503afaaef25ea92efe5ed61ee70333a/u);
    assert.match(source, /33021840645/u);
    assert.match(source, /98356728084/u);
    assert.match(source, /98358338163/u);
    assert.match(source, /98357986177/u);
    assert.match(source, /THREE INDEPENDENT QA EVIDENCE JOIN GAPS \/ HOSTED EXACT-FRAME SUCCESSOR ARM \+ DEPLOYMENT EXACT-FRAME EVIDENCE ARM \+ PHASE_G DIRECT-CONTACT OBSERVABLE JOIN \/ DESIGN_CHANGE_REQUIRED/u);
    assert.match(source, /COHERENT QA EVIDENCE JOIN PACKET \/ SAME_RAF EXACT-FRAME SUCCESSOR ARM FOR HOSTED\+DEPLOYMENT \+ STRICT ATOMIC EXACT-ACTOR DIRECT-CONTACT RECEIPT \/ QA_HARNESS_ONLY/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 34|material-iteration-34/u);
    assert.match(source, /workflow-only iteration 35/u);
    assert.match(source, /Section 114/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  for (const token of [
    ...
    "9627424629",
    "9627655509",
    "bbc0b67eb9a3bb5a276ddbf015d873389fae91885a30d4a1cc206a99dd7a546c",
    "7964aef954a88d310b0fb082768ed133ba8bc882d0ee89438e9471d934019967",
    "9d8070361713ee3f0aed2d104c824709d34c4224dd0802841a9f8df35698cd38",
    "188bee2042466b0e2cceac7e6a90e76acc0d4a7b3caadf5c0a9d9f91316bb2cc",
    "5c977b99e1d58cd87047aa541ec5b1ec31bbeb89c964dbd357ff6833d72882bd",
    "a9f31a31b52ed365df7afa18786ec3fe87b3b3ec2fa325f546b931dde77598ed",
    "d9601d96dd13f7fbcdb5e963ed779daec17388783d8c6cff283001fc47083137",
  ]) assert.match(r98, new RegExp(token, "u"));
  for (const path of [
    "scripts/v0995-visual-integrity-browser-smoke.mjs",
    "scripts/v099-final-remediation-browser-smoke.mjs",
    "scripts/v100-phase-g-production-matrix.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r98, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r98, /v100-phase-g-exact-actor-direct-contact\/v1/u);
  assert.match(r98, /exact-actor-direct-contact/u);
  assert.match(r98, /same production snapshot/u);
  assert.match(r98, /release-anchor fallback is ineligible/u);
  assert.match(r98, /actual authored source attack presentation/u);
  assert.match(r98, /actual identity-bound target reaction/u);
  assert.match(r98, /same-page first-`suppressedRenderFrames > 0` observation/u);
  assert.match(r98, /test: close r98 exact-frame and direct-contact joins/u);
  assert.match(r98, /ten exact blobs/u);
  assert.match(r98, /exactly one `force: false` ref update/u);
  assert.match(r98, /No eleventh path/u);
  assert.match(r98, /\.github\/workflows\/ci\.yml/u);
  assert.match(r98, /tests\/ci-contract\.test\.mjs/u);
  assert.match(r98, /npm run qa:v100-phase-g/u);
  assert.match(r98, /High ambiguity: 0/u);
  assert.match(r98, /Medium ambiguity: 0/u);
  for (const source of [r99, r99Handoff]) {
    assert.match(source, /955bf287fa305d3d940a56a15bdb203f062ca27c/u);
    assert.match(source, /4b678ec0e503afaaef25ea92efe5ed61ee70333a/u);
    assert.match(source, /R99_ACCEPTED_PROOF_SCREENSHOT_ORDER_PACKET_ACTIVE/u);
    assert.match(source, /QA_HARNESS_ACCEPTED_PROOF_TO_SCREENSHOT_ORDERING/u);
    assert.match(source, /IMMEDIATE ACCEPTED_PROOF SCREENSHOT \+ RELEASE_DEADLINE RECEIPT BEFORE OBSERVER_STOP FINALLY CLEANUP/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 34|material-iteration-34/u);
    assert.match(source, /workflow-only iteration 35/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
  ...
  for (const token of [
    "67e7466c667eaf8c9e957976693e877148bb0ef736f149dc5b1243c82fef65d8",
    "bac2531c0f6a03f3e3c7c2c780a9cc1e5a71cfa76803591fb04c47f91e1453e2",
    "b93fcdb668ef151acfa048f60df4e2807bdddb5287b1e67c9d0d26cd24d768a9",
    "6a2bc5f278cde626b9eea0eb9df7393179704bd91fb7a2f2621f2bd3e4a86a10",
    "1c9381abaa1f32ece032e687e65fd9450e050a42f90629378fa3aee2a857928c",
  ]) assert.match(r99, new RegExp(token, "u"));
  assert.match(r99, /phase-g\/causal-proof/u);
  assert.match(r99, /phase-g\/observer-stop/u);
  assert.match(r99, /phase-g\/production-screenshot/u);
  assert.match(r99, /3,305 ms/u);
  assert.match(r99, /351/u);
  assert.match(r99, /causal-proof -> production-screenshot -> release-deadline receipt -> observer-stop -> overflow-audit/u);
  assert.match(r99, /test: close r99 accepted-proof screenshot ordering/u);
  assert.match(r99, /exactly one `force: false` ref update/u);
  assert.match(r99, /No eleventh path/u);
  assert.match(r99, /High ambiguity: 0/u);
  assert.match(r99, /Medium ambiguity: 0/u);
  for (const source of [r100, r100Handoff]) {
    assert.match(source, /955bf287fa305d3d940a56a15bdb203f062ca27c/u);
    assert.match(source, /4b678ec0e503afaaef25ea92efe5ed61ee70333a/u);
    assert.match(source, /R100_PAGE_OWNED_PHASE_G_EVIDENCE_STATE_MACHINE_PACKET_ACTIVE/u);
    assert.match(source, /QA_HARNESS_DISTRIBUTED_MUTABLE_EVIDENCE_STATE/u);
    assert.match(source, /NORMAL_RELEASE_CANDIDATE_INVALIDATION_UNMODELED/u);
    assert.match(source, /STRUCTURAL_PAGE_OWNED_PHASE_G_EVIDENCE_STATE_MACHINE/u);
    assert.match(source, /EXPLICIT_CANDIDATE_INVALIDATION/u);
    assert.match(source, /IMMUTABLE_ACCEPTED_WITNESS/u);
    assert.match(source, /SEALED_CAPTURE_TRANSACTION/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 34|material-iteration-34/u);
    assert.match(source, /workflow-only iteration 35/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
  ...
  for (const token of [
    "5433055306",
    "93f6847e6053d961e81335244cefa717da4789bb75f98af96c4e9f7dd8f9b073",
    "2d68ae8e840474ccf4312ecb0e5766397fcf63a50311fa61b910a0a8983f655a",
    "8f4b7846a2254bac565ffab1d59284e362ccebf1a384ad3fdb067c2eb3e78c08",
    "7622585fd5d6ad9f6273173cc6345ffa6f470ee84dcc2d9373f139b1130c42a0",
    "v100-phase-g-post-quiescence-proof/v6",
    "v100-phase-g-capture-transaction/v1",
    "OBSERVING -> WITNESS_ACCEPTED -> SCREENSHOT_RECEIPT_ACCEPTED -> CLEANED",
    "FAILED -> CLEANED_AFTER_FAILURE",
    "test: close r100 page-owned Phase G evidence state machine",
  ]) assert.match(r100, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  for (const path of [
    "scripts/v100-phase-g-production-matrix.mjs",
    "tests/v100-phase-g-checkpoint.test.mjs",
    "tests/v100-r11-combat-causal-history.test.mjs",
    ...
    "docs/PROJECT_STATE.md",
    "tests/v100-design-lock.test.mjs",
  ]) assert.match(r100, new RegExp(path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r100, /state ownership/u);
  assert.match(r100, /observer lifecycle/u);
  assert.match(r100, /presentation quiescence/u);
  assert.match(r100, /checkpoint ownership/u);
  assert.match(r100, /causal witness/u);
  assert.match(r100, /screenshot ordering/u);
  assert.match(r100, /cleanup ordering/u);
  assert.match(r100, /evidence persistence/u);
  assert.match(r100, /browser\/process lifecycle/u);
  assert.match(r100, /accepted proof -> one production screenshot -> actual after-screenshot/u);
  assert.match(r100, /OBSERVING \| WITNESS_ACCEPTED \| SCREENSHOT_RECEIPT_ACCEPTED -> FAILED/u);
  assert.match(r100, /same original target ID/u);
  assert.match(r100, /strictly greater than the frozen late-windup anchor/u);
  assert.match(r100, /supporting-prerequisite/u);
  assert.match(r100, /WAITING_SEQUENCE/u);
  assert.match(r100, /does not add a target lock or second windup requirement/u);
  assert.match(r100, /0\.8833333333333334/u);
  assert.match(r100, /1 \/ 12/u);
  assert.match(r100, /exactly one `force: false` ref update/u);
  assert.match(r100, /No `app\/\*\*`/u);
  assert.match(r100, /first red or uncertainty/u);
  assert.match(r100, /High ambiguity: 0/u);
  assert.match(r100, /Medium ambiguity: 0/u);
  for (const source of [r101, r101Handoff]) {
    assert.match(source, /955bf287fa305d3d940a56a15bdb203f062ca27c/u);
    assert.match(source, /4b678ec0e503afaaef25ea92efe5ed61ee70333a/u);
    assert.match(source, /R101_STRICT_V6_RECEIPT_DOMAIN_ACTIVE/u);
    assert.match(source, /QA_HARNESS_V6_NUMERIC_RECEIPT_DOMAIN_FAIL_OPEN/u);
    assert.match(source, /JAVASCRIPT_NUMBER_COERCION_ACCEPTS_NULL_AS_ZERO/u);
    assert.match(source, /STRICT_FINITE_NUMBER_RECEIPT_DOMAIN/u);
    assert.match(source, /HOST_AND_PAGE_SCHEMA_BOUNDARY_VALIDATION/u);
    assert.match(source, /EXHAUSTIVE_NEGATIVE_SUBSTITUTION_MATRIX/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 34|material-iteration-34/u);
    assert.match(source, /workflow-only iteration 35/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT|single final Producer checkpoint/u);
  }
  for (const token of [
    "5433664869",
    "5433678534",
    "tests/v100-phase-g-checkpoint.test.mjs:743",
    ...
    "Infinity",
    "-Infinity",
    "test: close r101 strict v6 receipt domain",
  ]) assert.match(r101, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  for (const owner of [
    "state ownership",
    "observer lifecycle",
    "presentation quiescence",
  ...
  ]) assert.match(r101, new RegExp(owner.replace("/", "\\/"), "u"));
  assert.match(r101, /exactly two realm-local helpers/u);
  assert.match(r101, /one-line audio guard therefore cannot be proven finite/u);
  assert.match(r101, /High ambiguity: 0/u);
  assert.match(r101, /Medium ambiguity: 0/u);
  for (const source of [r102, r102Handoff]) {
    assert.match(source, /955bf287fa305d3d940a56a15bdb203f062ca27c/u);
    assert.match(source, /4b678ec0e503afaaef25ea92efe5ed61ee70333a/u);
    assert.match(source, /R102_SHARED_V6_TEST_CONTRACT_ACTIVE/u);
    assert.match(source, /QA_HARNESS_V6_CONTRACT_OWNERSHIP_SPLIT/u);
    assert.match(source, /STATIC_LITERAL_ASSERTION_STALE/u);
    assert.match(source, /CROSS_SUITE_VALID_FIXTURE_SCHEMA_DRIFT/u);
    assert.match(source, /SHARED_V6_TEST_CONTRACT_OWNER/u);
    assert.match(source, /SEMANTIC_SOURCE_ASSERTION/u);
    assert.match(source, /CANONICAL_VALID_FIXTURE_FACTORY/u);
    assert.match(source, /REQUIRED_NUMERIC_PATH_CATALOG/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 34|material-iteration-34/u);
    assert.match(source, /workflow-only iteration 35/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
  ...
  for (const token of [
    ...
    "5433884179",
    "17/19",
    "tests/fixtures/v100-phase-g-v6-contract.mjs",
    "354897",
    "c3c1fffd7bba54a3243a8463db390e4b8c0354a0b9bbf66925de49334076e9eb",
    "exactly eleven paths",
    "test: close r102 shared v6 contract ownership",
  ]) assert.match(r102, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r102, /High ambiguity: 0/u);
  assert.match(r102, /Medium ambiguity: 0/u);
  for (const source of [r103, r103Handoff]) {
    assert.match(source, /955bf287fa305d3d940a56a15bdb203f062ca27c/u);
    assert.match(source, /4b678ec0e503afaaef25ea92efe5ed61ee70333a/u);
    assert.match(source, /R103_COMPLETE_STATIC_CONTRACT_AUDIT_ACTIVE/u);
    assert.match(source, /QA_HARNESS_PHASE_G_STATIC_CONTRACT_OBSERVABILITY_FAILURE/u);
    assert.match(source, /MONOLITHIC_FAIL_FAST_MASKING/u);
    assert.match(source, /STALE_FAILURE_REGION_ANCHOR/u);
    assert.match(source, /COMPLETE_STATIC_CONTRACT_AUDIT/u);
    assert.match(source, /COLLECT_ALL_ASSERTION_RECEIPTS/u);
    assert.match(source, /STABLE_FAILURE_STATE_REGION_OWNER/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 34|material-iteration-34/u);
    assert.match(source, /workflow-only iteration 35/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
  ...
  for (const token of [
    ...
    "354897",
    "c3c1fffd7bba54a3243a8463db390e4b8c0354a0b9bbf66925de49334076e9eb",
    "exactly the Section 118 eleven paths",
    "test: close r103 complete static contract audit",
  ]) assert.match(r103, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r103, /High ambiguity: 0/u);
  assert.match(r103, /Medium ambiguity: 0/u);
  for (const source of [r104, r104Handoff]) {
    assert.match(source, /955bf287fa305d3d940a56a15bdb203f062ca27c/u);
    assert.match(source, /4b678ec0e503afaaef25ea92efe5ed61ee70333a/u);
    assert.match(source, /R104_SINGLE_SOURCE_CAUSAL_EVENT_MACHINE_ACTIVE/u);
    assert.match(source, /QA_HARNESS_PHASE_G_CAUSAL_EVENT_STATE_COLLAPSE/u);
    assert.match(source, /SOURCE_COMMIT \+ AUTHORED_TRAVEL \+ TARGET_CONTACT_REACTION_FORCED_IN_ONE_40MS_OBSERVER_SNAPSHOT/u);
    assert.match(source, /SINGLE_SOURCE_V7_CAUSAL_EVENT_MACHINE/u);
    assert.match(source, /SOURCE_COMMITTED -> IMPACT_PENDING -> WITNESS_ACCEPTED/u);
    assert.match(source, /IDENTICAL NODE\/PAGE REDUCER/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 34|material-iteration-34/u);
    assert.match(source, /workflow-only iteration 35/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
  ...
  for (const token of [
    ...
    "3/3",
    "Stage 6",
    "EXACT_DIRECT_CONTACT_CAUSAL_RECEIPT_MISSING",
    "scripts/v100-phase-g-proof-machine.mjs",
    "tests/v100-phase-g-proof-machine.test.mjs",
    "exactly these thirteen paths",
    "test: close r104 single-source causal event machine",
    "syntax `4/4`",
    "focused source `32/32`",
    "proof-machine `13/13`",
    "canonical six-file command `55/55`",
  ]) assert.match(r104, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r104, /High ambiguity: 0/u);
  assert.match(r104, /Medium ambiguity: 0/u);
  for (const source of [r105, activeHandoff, currentCursor]) {
    assert.match(source, /2c9df5892503deb3ce5652ef2adfce34e4353e1c/u);
    assert.match(source, /fefb661a224452e081fce91b12553ed4d3da12ad/u);
    assert.match(source, /R105_STATIC_PROBE_TRANSPORT_OWNERSHIP_ACTIVE/u);
    assert.match(source, /QA_HARNESS_STATIC_PROBE_PROCESS_TRANSPORT/u);
    assert.match(source, /UNBOUNDED COMBINATORIAL JSON ENVIRONMENT PAYLOAD \+ SPAWNSYNC ERROR ELISION/u);
    assert.match(source, /STRUCTURAL STATIC_PROBE_SIMPLIFICATION/u);
    assert.match(source, /IN_PROCESS SINGLE_SOURCE COMBINATORIAL REDUCER/u);
    assert.match(source, /BOUNDED PRODUCTION_PROCESS PARITY PROBES \+ TRANSPARENT SPAWN FAILURE/u);
    assert.match(source, /NEXT_OWNER`: `SOL_REMEDIATION`/u);
    assert.match(source, /material iteration 35|material-iteration-35/u);
    assert.match(source, /SOL_FINAL_REVIEW/u);
    assert.match(source, /FINAL PRODUCER RELEASE-CANDIDATE CHECKPOINT/u);
  }
  for (const token of [
    ...
    "tests/v100-phase-g-checkpoint.test.mjs",
    "tests/v100-design-lock.test.mjs",
    "test: close r105 static probe transport ownership",
    "exact-five",
    "focused source `32/32`",
    "canonical six-file command `55/55`",
  ]) assert.match(r105, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&"), "u"));
  assert.match(r105, /High ambiguity: 0/u);
  assert.match(r105, /Medium ambiguity: 0/u);
  assert.match(currentCursor, /Actual r104 transport \/ CI #947 return \/ r105 structural design/u);
  assert.match(currentCursor, /Actual r103 runtime return \/ r104 causal event-machine design/u);
  assert.match(currentCursor, /Actual r102 source return \/ r103 complete static-contract design/u);
  assert.match(currentCursor, /Actual r101 source return \/ r102 structural test-contract design/u);
  assert.match(currentCursor, /Actual r100 source return \/ r101 design/u);
  assert.match(currentCursor, /Actual r99 local return \/ r100 structural design/u);
  assert.match(currentCursor, /Actual r98 local return \/ r99 design/u);
  assert.match(currentCursor, /Actual r97 transport \/ CI #946 return \/ r98 design/u);
  assert.match(currentCursor, /Actual r96 transport \/ CI #945 return \/ r97 design/u);
  assert.match(currentCursor, /Actual r95 transport \/ CI #944 return \/ r96 design/u);
  assert.match(currentCursor, /Actual r94 transport return \/ r95 design/u);
  assert.match(currentCursor, /Actual r93 transport return \/ r94 design/u);
  assert.match(currentCursor, /Actual r92 transport \/ CI #943 official-outage return \/ r93 design/u);
  assert.match(currentCursor, /Actual r91 local acceptance \/ r92 read-back/u);
  assert.match(currentCursor, /Actual r90 pre-implementation source return \/ r91 design/u);
  assert.match(currentCursor, /Actual r89 local return \/ r90 design/u);
  assert.match(currentCursor, /Actual r88 automatic focused return \/ r89 design/u);
  assert.match(currentCursor, /Actual r87 transport return \/ r88 design/u);
  assert.match(currentCursor, /Actual r86 source return \/ r87 design/u);
  assert.match(currentCursor, /Actual r85 transport return \/ r86 design/u);
  ...
  assert.match(currentCursor, /Actual r76 local acceptance \/ r77 read-back/u);
  assert.match(currentCursor, /Actual r77 remote return \/ r78 design/u);
  assert.match(currentCursor, /Actual r78 local acceptance \/ r79 read-back/u);
  ...
  assert.match(currentCursor, /R74_REMOTE_STAGE25_VALID_SAMPLE_COUNT`: `57` of 57 over `10997` ms/u);
  assert.match(currentCursor, /R74_REMOTE_STAGE25_GENERIC_CAUSAL_GREEN_COUNT`: `4` of 4 axes/u);
  assert.match(currentCursor, /R74_REMOTE_PHASE_G_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R75_LOCAL_ORDERED_STAGE6_GREEN_COUNT`: `1` at ordered position 1/u);
  assert.match(currentCursor, /R75_LOCAL_ORDERED_STAGE24_GREEN_COUNT`: `1` at ordered position 2/u);
  assert.match(currentCursor, /R75_LOCAL_STAGE25_POST_COMMIT_TARGET_SOURCE_FAILURE_COUNT`: `1` at ordered position 3/u);
  assert.match(currentCursor, /R75_LOCAL_STAGE25_RELEASE_TO_SEQUENCE_COMMIT_MS`: `265`/u);
  assert.match(currentCursor, /R75_LOCAL_STAGE25_GENERIC_CAUSAL_GREEN_COUNT`: `4` of 4 axes/u);
  assert.match(currentCursor, /R75_LOCAL_PHASE_G_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R77_REMOTE_STAGE25_TERMINAL_HANDOFF_FAILURE_COUNT`: `1` at ordered position 3/u);
  assert.match(currentCursor, /R77_REMOTE_DEPLOYMENT_736_SETUP_PRESENTATION_CRASH_COUNT`: `1` at Mayo asset-boundary sealing/u);
  assert.match(currentCursor, /R77_REMOTE_DEPLOYMENT_736_D_STATE_SAMPLE_COUNT`: `35`；`anon_pipe_write` `34`；OOM／fatal `0`/u);
  assert.match(currentCursor, /R77_REMOTE_DEPLOYMENT_GREEN_VIEWPORT_COUNT`: `5` of 6/u);
  assert.match(currentCursor, /R77_REMOTE_CANONICAL_HUD_DEPENDENCY_SKIPPED_COUNT`: `1`/u);
  assert.match(currentCursor, /R77_REMOTE_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R78_LOCAL_DEPLOYMENT_736_GREEN_UNIT_COUNT`: `8` of `8`/u);
  assert.match(currentCursor, /R78_LOCAL_DEPLOYMENT_736_GREEN_CHECKPOINT_COUNT`: `48` of `48`/u);
  assert.match(currentCursor, /R78_LOCAL_DEPLOYMENT_ARTIFACT_INTEGRITY_GREEN_COUNT`: `56` of `56`/u);
  assert.match(currentCursor, /R78_LOCAL_STAGE25_STANDALONE_GREEN_COUNT`: `3` of `3`/u);
  assert.match(currentCursor, /R78_LOCAL_ORDERED_PHASE_G_GREEN_COUNT`: `3` of `3`/u);
  assert.match(currentCursor, /R78_LOCAL_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R79_REMOTE_PHASE_G_POST_CONVERGENCE_PAGE_CRASH_COUNT`: `1` at ordered Stage 25/u);
  assert.match(currentCursor, /R79_REMOTE_HOSTED_MUTABLE_SCREENSHOT_BACKPRESSURE_COUNT`: `1` at mission/u);
  assert.match(currentCursor, /R79_REMOTE_DEPLOYMENT_QA_PIXEL_AUDIT_ATOMIC_FAILURE_COUNT`: `1` at WebKit 844x340/u);
  assert.match(currentCursor, /R79_REMOTE_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R80_LOCAL_SOURCE_STATIC_BUILD_GREEN_COUNT`: `1` complete final-byte sequence/u);
  assert.match(currentCursor, /R80_LOCAL_STAGE25_STANDALONE_GREEN_COUNT`: `1` at process 1 of 3/u);
  assert.match(currentCursor, /R80_LOCAL_STAGE25_CROSS_SNAPSHOT_HANDOFF_FAILURE_COUNT`: `1` at process 2 of 3/u);
  assert.match(currentCursor, /R80_LOCAL_LATER_RUNTIME_AXIS_EXECUTED_COUNT`: `0`/u);
  assert.match(currentCursor, /R80_LOCAL_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R81_LOCAL_SOURCE_STATIC_BUILD_GREEN_COUNT`: `1` complete final-byte sequence/u);
  assert.match(currentCursor, /R81_LOCAL_STAGE25_STANDALONE_GREEN_COUNT`: `3` of `3` first attempts/u);
  assert.match(currentCursor, /R81_LOCAL_ORDERED_PHASE_G_GREEN_COUNT`: `3` of `3` stages on the first ordered attempt/u);
  assert.match(currentCursor, /R81_LOCAL_HOSTED_READY_GREEN_COUNT`: `3` of `3`; fault `35` of `35`; mission final-canvas `15` of `15`; mutable screenshot `45` of `45`/u);
  assert.match(currentCursor, /R81_LOCAL_DEPLOYMENT_844_GREEN_UNIT_COUNT`: `8` of `8`; checkpoint `48` of `48`; artifact `56` of `56`/u);
  assert.match(currentCursor, /R81_LOCAL_COMPLETE_TEST_GREEN_COUNT`: `1197` of `1197`; lint errors `0`; content／final build green/u);
  assert.match(currentCursor, /R81_LOCAL_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R73_LOCAL_CROSS_UNIT_RESUME_RECEIPT_GREEN_COUNT`: `7` of 7/u);
  assert.match(currentCursor, /Actual r65 local acceptance/u);
  assert.match(currentCursor, /Actual r65 remote return/u);
  assert.match(currentCursor, /R65_LOCAL_ORDERED_PHASE_G_GREEN_COUNT/u);
  assert.match(currentCursor, /R65_REMOTE_STAGE6_POST_RELEASE_PRESENTATION_CRASH_COUNT/u);
  assert.match(currentCursor, /R65_REMOTE_DEPLOYMENT_1280_PRE_FIXTURE_PRESENTATION_CRASH_COUNT/u);
  assert.match(currentCursor, /R65_REMOTE_CANONICAL_HUD_DEPENDENCY_SKIPPED_COUNT/u);
  assert.match(currentCursor, /Actual r66 local source\/runtime return/u);
  assert.match(currentCursor, /R66_LOCAL_DEPLOYMENT_DIAGNOSTIC_OPERATION_OWNERSHIP_FAILURE_COUNT/u);
  assert.match(currentCursor, /R66_LOCAL_DEPLOYMENT_REMAINING_UNIT_NOT_RUN_COUNT/u);
  assert.match(currentCursor, /R66_LOCAL_ORDERED_PHASE_G_NOT_RUN_COUNT/u);
  assert.match(currentCursor, /Actual r67 source return/u);
  assert.match(currentCursor, /R67_LOCAL_RUNTIME_EVIDENCE_SOURCE_PASS_COUNT/u);
  assert.match(currentCursor, /R67_LOCAL_SOURCE_NEGATIVE_SCOPE_ALIAS_FAILURE_COUNT/u);
  assert.match(currentCursor, /R69_LOCAL_DEPLOYMENT_1280_GREEN_COUNT/u);
  assert.match(currentCursor, /R69_LOCAL_ORDERED_STAGE6_PRE_RELEASE_CUE_OWNER_FAILURE_COUNT/u);
  assert.match(currentCursor, /R69_LOCAL_ORDERED_STAGE24_NOT_RUN_COUNT/u);
  assert.match(currentCursor, /R69_LOCAL_ORDERED_STAGE25_NOT_RUN_COUNT/u);
  assert.match(currentCursor, /Actual r68 Design source return/u);
  assert.match(currentCursor, /R68_LOCAL_DESIGN_LOCK_PASS_COUNT/u);
  assert.match(currentCursor, /R68_ACTIVE_RELEASE_TOKEN_OMISSION_COUNT/u);
  assert.match(currentCursor, /Actual r71 local acceptance/u);
  assert.match(currentCursor, /Actual r71 remote return/u);
  assert.match(currentCursor, /R71_LOCAL_ORDERED_PHASE_G_GREEN_COUNT/u);
  assert.match(currentCursor, /R71_LOCAL_STAGE6_IMMUTABLE_LATER_DEATH_ACCEPTANCE_COUNT/u);
  assert.match(currentCursor, /R71_LOCAL_ORDERED_RETRY_RERUN_COUNT`: `0`/u);
  assert.match(currentCursor, /R71_REMOTE_PR_VERIFY_CROSS_UNIT_PAUSE_HANDOFF_FAILURE_COUNT`: `1` job／`6` of six Chromium viewports/u);
  assert.match(currentCursor, /R71_REMOTE_PHASE_G_DEPENDENCY_SKIPPED_COUNT`: `1`/u);
  assert.match(currentCursor, /Actual r72 source return/u);
  assert.match(currentCursor, /R72_LOCAL_RUNTIME_EVIDENCE_SOURCE_PASS_COUNT`: `2` of 3/u);
  assert.match(currentCursor, /R72_LOCAL_SOURCE_NEGATIVE_ASSIGNMENT_ALIAS_FAILURE_COUNT`: `1`/u);
