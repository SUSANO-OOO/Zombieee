...
test("production visual manifest uses dedicated title, command, all stages, and event cuts", async () => {
  const paths = [
  ...
  assert.equal(Object.keys(PRODUCTION_VISUALS.stages).length, 30);
  assert.equal(paths.length, 3 + Object.keys(PRODUCTION_VISUALS.stages).length + 3);
  assert.equal(new Set(paths).size, paths.length);
  const hashes = [];
  for (const path of paths) {
    assert.match(path, /^\/art\/v(?:060|070|075|080|090|100)\/(?:[a-z0-9-]+\/)*[a-z0-9-]+\.webp$/);
