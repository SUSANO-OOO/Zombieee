import {createV100FilteredImageLayer} from './v100FilteredImageLayer.js';
...
export function createV100ImageSampler({maxBytes=12*1024*1024,maxEntries=128,maxWorkingBytes=8*1024*1024,createCanvas=()=>document.createElement('canvas')}={}){
 ...
 const cache=new Map();
 const sampledCanvases=new WeakSet();
 const filteredLayer=createV100FilteredImageLayer({createCanvas:()=>createCanvas(),isImmutableSource:image=>
  (typeof HTMLImageElement!=='undefined'&&image instanceof HTMLImageElement)
  ||(typeof ImageBitmap!=='undefined'&&image instanceof ImageBitmap)
  ||sampledCanvases.has(image)||image?.__V100_IMMUTABLE_SAMPLER__===true});
 const paint=(ctx,image,args)=>typeof ctx.filter==='string'&&ctx.filter!=='none'
  ?filteredLayer.draw(ctx,image,...args):ctx.drawImage(image,...args);
 const dispose=entry=>{entry.canvas.width=0;entry.canvas.height=0;};
 function clear(){for(const entry of cache.values())dispose(entry);cache.clear();identities=new WeakMap();bytes=0;filteredLayer.clear();}
 function snapshot(){return{entries:cache.size,bytes,maxBytes,hits,builds,evictions,filteredImageLayer:filteredLayer.snapshot()};}
 function drawTexture(ctx,image,args,nearestLevel){
  ...
  if(rect.length!==8||!rect.every(Number.isFinite)){paint(ctx,image,args);return;}
  ...
  if(!(sw>0&&sh>0&&pixelWidth>0&&pixelHeight>0)||!Number.isFinite(pixelWidth+pixelHeight)||!ctx.imageSmoothingEnabled){paint(ctx,image,args);return;}
  ...
  if(!steps.length||maxEntries<1||cost>maxBytes||steps[0].w*steps[0].h*4>maxWorkingBytes){paint(ctx,image,args);return;}
  ...
  if(entry){hits++;cache.delete(key);cache.set(key,entry);}
   ...
   try{
    for(const [index,size] of steps.entries()){
     const canvas=createCanvas();sampledCanvases.add(canvas);current=canvas;canvas.width=size.w;canvas.height=size.h;
  ...
  paint(ctx,entry.canvas,[0,0,width,height,dx,dy,dw,dh]);
 }
