...
export function V100ModesView({ save, onBack, onLoadout, onSave }: Props) {
  const [tab, setTab] = useState<"outbreak" | "compendium" | "records">("outbreak");
  ...
  return <section className="v100-panel v100-modes-screen" data-v100-surface="modes" aria-label="異常発生・記録">
    ...
    <nav className="v100-equipment-tabs" aria-label="記録の種類">{([ ["outbreak", "異常発生"], ["compendium", "ボス図鑑"], ["records", "戦績"] ] as const).map(([id, name]) => <button type="button" key={id} aria-pressed={tab === id} onClick={() => setTab(id)}>{name}</button>)}</nav>
    ...
    {bosses.length === 0 && <p>物語で初めて撃破した異常個体が、ここに記録されます。</p>}
    <div className="v100-equipment-catalog">{bosses.map(boss => {
      ...
      return <article className="v100-equipment-card" key={boss.id} data-outbreak-boss-id={boss.id}>
        ...
        <p>総撃破 {defeatCounts[boss.id] ?? 0}回 / 異常発生制圧 {clears}回</p>
