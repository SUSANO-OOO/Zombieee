...
const baseRoot = process.argv[2] ? path.resolve(process.argv[2]) : null;
const evidenceRoot = path.resolve(
  process.env.P5_QA_EVIDENCE_DIR ?? "outputs/p5-stage3-final-bounded",
);
...
function emptyDiagnostics(diagnostics = {}) {
  return ["consoleErrors", "pageErrors", "requestFailures", "failedRequestDetails", "httpErrors"]
    .every((key) => (diagnostics[key] ?? []).length === 0)
    && Number(diagnostics.pendingRequestCount ?? 0) === 0;
}

function isRetryableTargetClosed(summary) {
