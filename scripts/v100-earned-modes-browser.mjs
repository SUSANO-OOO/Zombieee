...
report.driver=await Promise.all(['scripts/v100-earned-modes-browser.mjs','scripts/v100-normal-tactical-input.mjs'].map(async path=>({path,sha256:createHash('sha256').update(await readFile(path)).digest('hex')})));
...
try{
 ...
 await shot('survival-result');await page.reload();await click('ブラウザで遊ぶ');await page.getByRole('region',{name:'防衛継続作戦の戦果',exact:true}).waitFor();assert.deepEqual(await save(),final);survival.status='passed';assert.deepEqual(report.errors,[]);report.status='passed';
}catch(e){report.status='failed';report.error=String(e.stack??e);await shot('failure').catch(()=>{});process.exitCode=1;}
finally{report.finalSave=await save().catch(()=>null);await context.storageState({path:out+'/browser-storage.json',indexedDB:true});await persist();await browser.close();console.log(JSON.stringify({status:report.status,error:report.error?.slice(0,900)}));}
