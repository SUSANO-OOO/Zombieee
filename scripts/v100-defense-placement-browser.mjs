...
const out=process.env.V100_DEFENSE_BROWSER_DIR??'outputs/v100-defense-placement-browser-r1';await mkdir(out,{recursive:false});
...
try{
  for(const [engine,api] of Object.entries({webkit,chromium}))for(const viewport of [{width:844,height:340},{width:844,height:390},{width:1280,height:720}])for(const stageNumber of [22,2,7,18]){
    ...
    try{
      await page.addInitScript(({origin,seed})=>{
        ...
        proto.drawImage=function(image,...a){
          ...
          if(this.canvas===document.querySelector('.game-shell canvas')&&/\/(defense-perimeter)-states-v1\.webp$/u.test(source)){
            ...
            if(/(defense-perimeter)-states/u.test(source))window.__OBJECTIVE_PLACEMENT__.objects=[];
      ...
      item.observed=await page.evaluate(()=>{
        const hud=document.querySelector('.defense-objective')?.getBoundingClientRect(),snapshot=window.__ASHFALL_BATTLE_QA__.getSnapshot();
